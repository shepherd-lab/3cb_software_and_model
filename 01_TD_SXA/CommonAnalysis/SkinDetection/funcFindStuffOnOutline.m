...
function [PreciseOutline,Surface,midcurve_p,midpoint]=funcFindStuffOnOutline(Outline)
 global  ROI  Outline Analysis
   ...
   % figure; plot(yt,xt,'b'); hold on;
    ...
    maxt = max(Outline.x);
    xtmax_indvect = find(Outline.x==maxt);  
    max_ind  = max(xtmax_indvect);
    min_ind = min(xtmax_indvect);
    
    xtmax_index = round((max_ind + min_ind)/2);
    Analysis.ROIbreast_midpoint = Outline.y(xtmax_index(1));
    %figure;plot(Outline.x, Outline.y,'b-', Outline.x(xtmax_index), Analysis.ROIbreast_midpoint,'*r');
...
if method==1  %former method
    ...
    %added to remove negatives
    Outline.x(Outline.x<=0) = 1;
    Outline.y(Outline.y<=0) = 1;
    ...
    while i<I
        ...
        if Outline.x(i-1)<Outline.x(i)
            while Outline.x(i)>Outline.x(i-1)
                ...
                I=I-1;
                i=min(i,numel(Outline.x));
                %FD 1/12/2012 Added to eliminate indexing error
            end
        end
        i=i+1;
        count3 = count3 + 1;
