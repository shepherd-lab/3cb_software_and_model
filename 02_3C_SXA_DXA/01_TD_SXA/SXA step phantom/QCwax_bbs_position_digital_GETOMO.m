% 07-11-2019 by sypks: ajustment for tomo acquired images
...
%     figure;plot(im_profileV);
   ...
   %figure;plot(im_profileH);
    ...
    plot(im_profileH);colormap(gray);
    if ~isempty(strfind(Info.StudyID, 'UVM-Selenia'))
   ...
   % funcBox(ROI.xmin,ROI.ymin,ROI.xmin+ROI.columns,ROI.ymin+ROI.rows,'b',2); 
    ...
    ry = [y3min y3min y3max y3max];
     wire_mask_temp = roipoly(wire_mask_temp,cx,ry);    %07012019 sypks not showing up with 3C03xxxx
   wire_mask = logical(imadd(wire_mask, wire_mask_temp));
...
%  figure;imagesc(im_Z); colormap(gray);
    Zs1 = imsubtract(Z,im_Z);   %07012019 sypks this line removes the 9 boxes containing simulated tissue material from the image
...
%figure;imagesc(Zs1); colormap(gray);
  ...
  %{  
   ...
   %}
    ...
    %% this needs to be address 07012019 sypks
