function group = groupingPairs_GE(b)
global Info
...
group(1).index = find(b(:,1) > quarter_X & b(:,1) < mid_X & b(:,2) < mid_Y)  %- 400*crop_coef) 5 7 ;
...
group(5).index = find(b(:,1) > mid_X+quarter_X & b(:,2) > mid_Y) %+ 40 bottom right 13 14
group(6).index = find(b(:,1)< quarter_X & b(:,2) > mid_Y) %+ 40 bottom left 2 4
group(7).index = find(b(:,1)< quarter_X & b(:,2) < mid_Y) %+ 40 up left 1 3
