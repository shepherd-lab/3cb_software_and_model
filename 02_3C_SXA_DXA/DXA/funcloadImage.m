...
pathstr = erase(pathstr,'png_files');
root_dir = pathstr;
len = length(name);
if len < 7
    patient_ID = name
...
fmat_name = [pathstr,'mat_files\',name,'.mat'] % change SM for file open presentation/Selenia raw (1:end-10)
s=dir(fmat_name);
if flag.slice == true
    MachineParams.dark_counts = 0;
    %old UCSF location
    %dicom_filename = '\\researchstg\aaStudies\Breast Studies\UCSF-GE Collaboration\GE_reconstruction\Projections\E11558554S1494I1.DCM';
    dicom_filename = '\SRL\aaStudies\Breast Studies\UCSF-GE Collaboration\GE_reconstruction\Projections\E11558554S1494I1.DCM';
    info_dicom = dicominfo(dicom_filename);
else
    if (~isdir(fname) & isdicom(fname))
        MachineParams.dark_counts = 0;
        info_dicom = dicominfo(fname);
    else
        if ~isempty(s)
            load(fmat_name);
...
if isfield(info_dicom,'ProtocolName')
    if ~isempty(strfind(lower(info_dicom.ProtocolName),lower('R MLO')))
        Info.ViewId = 4;
    elseif ~isempty(strfind(lower(info_dicom.ProtocolName),lower('L MLO')))
        Info.ViewId = 5;
    elseif ~isempty(strfind(lower(info_dicom.ProtocolName),lower('R CC')))
        Info.ViewId = 2;
    elseif ~isempty(strfind(lower(info_dicom.ProtocolName),lower('L CC')))
        Info.ViewId = 3;
    else
        Info.ViewId = 1;
    end
else
    if ~isempty(strfind(lower(info_dicom.SeriesDescription),lower('R MLO')))
        Info.ViewId = 4;
    elseif ~isempty(strfind(lower(info_dicom.SeriesDescription),lower('L MLO')))
        Info.ViewId = 5;
    elseif ~isempty(strfind(lower(info_dicom.SeriesDescription),lower('R CC')))
        Info.ViewId = 2;
    elseif ~isempty(strfind(lower(info_dicom.SeriesDescription),lower('L CC')))
        Info.ViewId = 3;
    else
        Info.ViewId = 1;
    end
...
elseif ~isempty(strfind(lower(info_dicom.ManufacturerModelName),lower('Seno')))
    Info.DigitizerId = 5;
    flag.Senograph = true;
...
%20190107 found this comment below useful, what is it .. sypks
...
if (Info.DigitizerId >= 4 | flag.Selenia_image == true) & flag.RawImage == false
    if Result.DXASelenia == true & Info.Database == false
        ...
        if isfield(info_dicom,'ImageComments')
    ...
    if ~isdir(fname) & isdicom(fname)
        Result.image=double(dicomread(fname));
        %         if Info.DigitizerId ~= 4
        %         Result.image = Result.image +  25000;
        %         end
    else
    ...
    if ~isempty(Result.image)
        ...
        elseif Result.DXASelenia == false
            ...
            if Info.Database == false %& Info.nomatfile == false
                %                 [pathstr,name,ext] = fileparts(fname)  %,versn SM05242016
                %                 fmat_name = [pathstr(1:end-9),'mat_files\',name,'.mat'];
                s=dir (fmat_name);
                if isempty(s)
                    %Result.image2=
                    %log_convert(Result.image-MachineParams.dark_counts);
                    %SM 05222017
                else
                    load(fmat_name);
