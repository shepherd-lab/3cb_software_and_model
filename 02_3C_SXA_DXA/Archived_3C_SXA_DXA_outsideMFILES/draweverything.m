       ...
       % end
        ...
        if ~DrawOption.DONTCHANGE
          ...
          % figure; imagesc(Image.image);colormap(gray);
           ...
           % colormap(funccomputecolormap(Image,Threshold,get(ctrl.ShowBackGround,'value')));
             colormap(gray);      %put the image in gray  
        end
       % hold off;
        ...
        if Threshold.ComputedCalc
                try
%                    plot((ROI.xmin+Threshold.boundary(:,2)-1)/Undersamplingfactor,(ROI.ymin+Threshold.boundary(:,1)-1)/Undersamplingfactor,'.r', 'LineWidth', 0.5,'markersize',0.5);
                     imagesc(Image.image.*maps.mask_calc2);colormap(gray); hold on;
                     plot((Threshold.boundary(:,2)-1)/Undersamplingfactor,(Threshold.boundary(:,1)-1)/Undersamplingfactor,'.r', 'LineWidth', 0.5,'markersize',5);
                catch
                end
         end
...
%                     FreeForm.FreeFormCluster(index).patch=plot((FreeForm.FreeFormCluster(index).face(:,1))/Undersamplingfactor,(FreeForm.FreeFormCluster(index).face(:,2))/Undersamplingfactor,'color',FreeFormColor,'ButtonDownFcn',['FreeFromButtonDownFct(',num2str(index),')'],'linewidth',FREEFORMPLOTSIZE);
                    ...
                    if sz_image(1) == ROI.rows
                       FreeForm.FreeFormCluster(index).patch=plot((FreeForm.FreeFormCluster(index).face(:,1))/Undersamplingfactor-ROI.xmin,(FreeForm.FreeFormCluster(index).face(:,2))/Undersamplingfactor-ROI.ymin,'color',FreeFormColor,'ButtonDownFcn',['FreeFromButtonDownFct(',num2str(index),')'],'linewidth',FREEFORMPLOTSIZE);
                    else
                       FreeForm.FreeFormCluster(index).patch=plot((FreeForm.FreeFormCluster(index).face(:,1))/Undersamplingfactor,(FreeForm.FreeFormCluster(index).face(:,2))/Undersamplingfactor,'color',FreeFormColor,'ButtonDownFcn',['FreeFromButtonDownFct(',num2str(index),')'],'linewidth',FREEFORMPLOTSIZE);
                    end
                else                                   
