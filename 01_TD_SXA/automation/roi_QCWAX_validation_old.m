...
if strcmpi(Analysis.film_identifier(1:3),'mar') | strcmpi(Analysis.film_identifier(1:3),'NC_')| strcmpi(Analysis.film_identifier(1:3),'Avo')| strcmpi (Info.machine_id,'21')|strcmpi (Info.machine_id,'22')|strcmpi (Info.machine_id,'23')|strcmpi (Info.machine_id,'24')|strcmpi (Info.machine_id,'25')|strcmpi (Info.machine_id,'26') 
...
%draweverything;
% another roi
roi_grscale.xmin = round(xmin);
roi_grscale.ymin = round(ymin);
if roi_grscale.ymin < 0 %JW roi_grscale.ymin in some cases negative
    roi_grscale.ymin = 1
end
roi_grscale.rows=ymax-ymin+1;
roi_grscale.columns=xmax-xmin+1;
...
for index=1:QCAnalysisData.Draw.Compartments
    ...
    Gen3Roi(index, 3) = mean(mean(Image.image(roi(index).ymin:roi(index).ymax, roi(index).xmin:roi(index).xmax)));
    Analysis.Step=8;
end     
...
% % %     DensityResults(8,index)=SXAAnalysis.roiDensityBFCorr;
% % %     DensityResults(9,index)=SXAAnalysis.roiDensity_246cmBFCorr;
% % %     a =1;
% % % end
% % % 
...
% % % Analysis.density_GEN3BFCorr = DensityROIResults(:,8);
% % % Analysis.density_GEN3_246cmBFCorr = DensityROIResults(:,9);
% % % 
...
for i = 1:3 %For each density, fit for VD vs. H
    ...
    plot(stepThick, Gen3Atnu, 'bo', thick_all, gen3_att(i,:), 'r-');
end
% hold off
