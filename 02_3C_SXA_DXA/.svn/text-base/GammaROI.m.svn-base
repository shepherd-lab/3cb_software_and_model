...
k = 150 / 140;
%k = 1;
Analysis.LeanHeightRefs = zeros(9,2);
...
thickness = -2;
    ...
    PhantomRefData.BoxX = round((185 + 2.5 * (thickness-3))* k); %205
    PhantomRefData.QuaterX = round(PhantomRefData.BoxX / 3)-1;
    PhantomRefData.QuaterY = round(PhantomRefData.BoxY / 3)-1;;
    PhantomRefData.Draw.MainBox= [PhantomRefData.BoxY+5,round(PhantomRefData.BoxX)* k +10];
    PhantomRefData.Draw.Compartments=9;
    PhantomRefData.Draw.BorderY= 17 * k;
    PhantomRefData.Draw.BorderX= 15 * k; %13
...
switch RequestedAction
    ...
    case 'InitDrawing'
      ...
      %} 
         J = imrotate(rot_image,-33,'bilinear'); %,'crop'28.85 -33
    ...
    case 'compute'
        ...
        for y_index=1:3
            for x_index=1:3
              if ~x_index %==1
                ...
                count = count + 1;
                %if count < 9
                   ref_values(count) =mean(mean(J(ROI.Y1:ROI.Y2,ROI.X1:ROI.X2)));
               % else
       ...
       % a =  FatGleanRefs(1:4,3);
        ...
        Excel('INIT');
        Excel('TRANSFERT',Analysis.LeanHeightRefs');
end