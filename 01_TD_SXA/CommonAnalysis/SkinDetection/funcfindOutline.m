...
loop_number = 0;
image(1:end,1:5)=1;
%FD 11/16/2011 added to remove problems with large images having zeros at
%the edge of the CCD. not a great solution.
while ok_continue %& loop_number 
    ...
    if (alpha<0.1)  %stop the process (faillure) %stop the process (faillure)FD 10/16/2011 Removed the time counter because it is processor dependent and also makes debugging hard.
        ok_continue=false;
        outline_x=[];
        outline_y=[];
        error=1;
...
%figure;plot(outline_x,outline_y,'b-');
a = 1;
