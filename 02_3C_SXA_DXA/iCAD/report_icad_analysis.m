...
xls_filename = '\\researchstg\aaData\Breast Studies\iCAD\SOP_patientid.xlsx';
...
for ipat=1:num_pats
    try
    num_xmls = length(pat{ipat}.icad_struct);
    PowerPoint('AddSlide');
    mass_name = [];
    mass_prob = [];
    clus_prob = [];
    ...
    len = length(pat{ipat}.icad_struct{1}.dcm_fname);      
        if len == 2
            dcm_name = char(pat{ipat}.icad_struct{1}.dcm_fname(2));
        else
            dcm_name = char(pat{ipat}.icad_struct{1}.dcm_fname);
        end
...
%     Position1(2) = Position(2);
    PowerPoint('addtext','text',['patient ID:   ',pat_cur],'position',[0.02 0.0],'bold',true,'underlined',true,'fontsize',1.2,'carriage',1);
...
%    Position = Position1;
    ...
    for j=1:num_xmls
        num_masses = pat{ipat}.icad_struct{j}.num_masses;
        num_clusters = pat{ipat}.icad_struct{j}.num_clusters
        
        len = length(pat{ipat}.icad_struct{j}.dcm_fname);
        
        if len == 2
            dcm_name = char(pat{ipat}.icad_struct{j}.dcm_fname(2));
        else
            dcm_name = char(pat{ipat}.icad_struct{j}.dcm_fname);
        end
        ...
        pres_image = imread(pres_imagename,'png');
        load(annofile_name);
        ...
        skin_points = pat{ipat}.icad_struct{j}.breast.skin_points;
        ...
        for ik=1:FreeForm.FreeFormnumber
            ...
            text(mean(FreeForm.FreeFormCluster(ik).face(:,1)),mean(FreeForm.FreeFormCluster(ik).face(:,2))+100,lesion_type,'Color','yellow','FontSize',8);
            else
            plot(FreeForm.FreeFormCluster(ik).face(:,1)*2,FreeForm.FreeFormCluster(ik).face(:,2)*2,'-y');hold on;
            text(mean(FreeForm.FreeFormCluster(ik).face(:,1)*2),mean(FreeForm.FreeFormCluster(ik).face(:,2)*2)+100,lesion_type,'Color','yellow','FontSize',8);
            end
        end
        ...
        if num_masses ~= 0
            for kn=1:num_masses
                mass_cent = pat{ipat}.icad_struct{j}.masses(kn).centroid;
                mass_line = pat{ipat}.icad_struct{j}.masses(kn).polyline;
                %                 plot(mass_cent(2), mass_cent(1), '.g');
                figure(h); plot(mass_line(:,2), mass_line(:,1), '-g'); hold on;
                 ...
                 text(mass_cent(2),mass_cent(1)-100,num2str(mass_prob(kn)),'Color','green','FontSize',8);hold on;
        ...
        if num_clusters ~= 0
            for km=1:num_clusters
                ucalcs_line = [];
                ucalcs_cent = [];
                num_ucalcs =  pat{ipat}.icad_struct{j}.calc(km).num_ucalcs;
                ...
                text(clus_cent(2),clus_cent(1)-100,num2str(clus_prob(km)),'Color','red','FontSize',8);hold on;                
                for kk = 1:num_ucalcs
                    ucalcs_cent = pat{ipat}.icad_struct{j}.calc(km).ucalcs(kk).ucalcs_cent;
                    %                     plot(ucalcs_cent(2), ucalcs_cent(1), '.r');
                    ucalcs_line = pat{ipat}.icad_struct{j}.calc(km).ucalcs(kk).polyline;
                    figure(h); plot(ucalcs_line(:,2), ucalcs_line(:,1), '-r'); hold on;
...
a = 1;
 ...
 if mod(ipat,100)==0 %& Info.SaveStatus ~= 0    
   PowerPoint('saveclose','text',[startdir_report,num2str(ipat),'.ppt']);
   PowerPoint('INIT');
 end
  catch
       error(lasterr)
