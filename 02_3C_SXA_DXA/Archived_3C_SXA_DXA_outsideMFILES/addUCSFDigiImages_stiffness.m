function addCPMCDigiImages3()

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%% written by Jeff Wang on September 30, 2008                   %%%%%
%%%%% function to add new CPMC mammograms to "mammo_CPMC" database %%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%% load site specific information
global Database;
Database.Name='mammo_CPMC';
load('\\researchstg\Working_Directory\Projects-Snap\aaSTUDIES\Breast Studies\BCSC\Analysis Code\Matlab\Versions\SXAVersion6.4_AddImagesToDatabase\CPMCdataloadDataSetup.mat');
%directory=cell2mat(loadDataSetup(1,1));
%directory='K:\aaData5\Breast Studies\CPMC\new images';
%directory='I:\aaData4\Breast Studies\CPMC\new images'; %SM change
directory=[cell2mat(loadDataSetup(1,1))];
destination=[cell2mat(loadDataSetup(2,1))];
newdestination=[cell2mat(loadDataSetup(4,1))];
Newdestination2=[cell2mat(loadDataSetup(5,1))] ; %New
newnewestdestination=[cell2mat(loadDataSetup(6,1))]; 
 
fileDirectory=[directory,'\*.mat'];
files=dir(fileDirectory);
% load('D:\Working_Directory\Projects-Snap\aaSTUDIES\Breast Studies\BCSC\Analysis Code\Matlab\Versions\SXAVersion6.4_AddImagesToDatabase\CPMCfiles.mat');
i=1;
errorCount=0;
H=clock;


while   1 % commented 03/22/2013   H(4)<18 && H(4) > 8
...
% % %         nalreadyinDB=size(alreadyinDB);
        ...
        if ~nalreadyinDB(1,1) > 0
            ...
            catch     parameter{	60	} = num2str(info_dicom_blinded.ExposureControlModeDescription);
            end
            ...
            field{1}='CPMC'; %study_id
            field{2}=info_dicom_blinded.AdmissionID; %patient_id / MRN
            field{3}=''; %visit_id
            field{4}=cell2mat(loadDataSetup(3,1)); %film_identifier
            ...
            %             room_id1=info_dicom_blinded.InstitutionName(len1-1:len1);
            %         end
            %         if strcmp(room_id,'1')|| strcmp(room_id1,'S1') %machine_id / room_id
            %             field{6}='1';
            %         elseif strcmp(room_id,'2')|| strcmp(room_id1,'S2')
            %             field{6}='2';
            %         elseif strcmp(room_id,'3')|| strcmp(room_id1,' 4')
            ...
            if strcmp(room_id,'H1KRHR83116e41') %room_id
                field{6}='0'; %CPMC StationName: selenia  %RM4 and more RM9???           
         ...
         % if ~isempty(viewID)
          % try field{8}=num2str(info_dicom_blinded.ExposureInuAs/1000); % Am changed 05032013 mAs
          % catch
          
          field {8}=num2str(info_dicom_blinded.Exposure);
...
%        
            ...
            newName=[name,'raw'];
            acq_date = info_dicom_blinded.StudyDate;
            %acq_date = info_dicom_blinded.AcquisitionDate;  %SM changed 032213
            len = length(acq_date);
            year = (acq_date(1,1:(len-4)));
            month = (acq_date(1,5:(len-2)));
            Day=(acq_date(1,7:(len-2)));
            folder = [year,'-',month];
            
            
% % %             if str2double(acq_date) < 20090101
% % %                 destination1 = [destination,folder];
% % %             elseif str2double(acq_date) < 20110701
% % %                 destination1 = [newdestination,folder];
% % %             elseif str2double(acq_date) < 20120601
% % %                 destination1=[newestdestination,folder];
% % %             else
% % %                 destination1 = [newnewestdestination,folder];
% % %             end
            ...
            elseif  str2double(acq_date) < 20130631
                destination1 = [newnewestdestination,folder];
            else str2double(acq_date) < 20131231
                
                 destination1 = [ Newdestination2,'\',folder];
            end
            ...
            source2=[directory,'\',newName,'.png'];
            try
                movefile(source1,'\\win\bbgd\aadata8\Lucyz-Desktop\Breast Studies\CPMC\duplicate filenames');
                movefile(source2,'\\win\bbgd\aadata8\Lucyz-Desktop\Breast Studies\CPMC\duplicate filenames');
            catch
            end
            i=i+1;
        end
