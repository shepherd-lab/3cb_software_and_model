function  Z4ComputeBreastDensityNewGEN3(roi_gen3,index)
    ...
    global Z4coef_tableBig Z4coef_tableSmall Z4coef_tableGE Result BreastMask MaskROIproj SXAAnalysis GEN3_thicknesses Image figuretodraw 
    global thickness_mapprojCrop DensityImageSkin DensityImage  DensityImage_246cm
     Error.DENSITY=false;
     ...
     SXAAnalysis = [];
            ...
            delta60 = GEN3_thicknesses(1)*0.1-6;       
            delta40 = GEN3_thicknesses(2)*0.1-4;   
             delta20 = GEN3_thicknesses(3)*0.1-2; 
...
% % %      delta20 = 3.29;
    ...
    % try
     ...
     if Info.DigitizerId >=5 | Info.centerlistactivated == 46 %| Info.centerlistactivated == 56  | Info.centerlistactivated == 53  %strcmp(Analysis.film_identifier(1:3),'mar')
         Analysis.Ibkg = extract_Ibkg(Database.Name, Info.acqDate, Info.kVp, ...
             Info.centerlistactivated, MachineParams.padSize);
     else
% % %          if Analysis.Ibkg<0             Temporary commented AM % 03042014
% % %              
% % %              Analysis.Ibkg = -2000;
% % %              
% % %          else
...
% % %                                 Info.centerlistactivated, MachineParams.padSize);
        %thickness is calculated thickness 
        ...
        elseif index == 3 || index == 6 || index == 9
          ...
          switch index
            case 1
              thickness =  (Analysis.thicknessWax1 - delta60);
            case 2
                thickness = (Analysis.thicknessWax2 - delta40);
            case 3
                thickness = (Analysis.thicknessWax3 - delta20);
            case 4
                thickness = ((Analysis.thicknessWax1+Analysis.thicknessWax4)/2 - delta60);
            case 5
                thickness = ((Analysis.thicknessWax2+Analysis.thicknessWax5)/2 - delta40);
            case 6
                thickness = ((Analysis.thicknessWax3+Analysis.thicknessWax3)/2 - delta20);
            case 7
                thickness =  (Analysis.thicknessWax4 - delta60);
            case 8
                thickness =  (Analysis.thicknessWax5 - delta40);
            case 9
                thickness =  (Analysis.thicknessWax6 - delta20);
            otherwise
                ;
          end
          
          if strcmpi(Info.machine_id,'20')  % Thickness 1 hasn't been captured after 20110919            
          ...
          floor_thick = floor(thickness_246cm);
          try  %%added by AM 02192014 
      ...
      if Info.DigitizerId >=5 %Info.centerlistactivated == 55 | Info.centerlistactivated == 56  | Info.centerlistactivated == 53  %strcmp(Analysis.film_identifier(1:3),'mar')                     
          Z4coef_table = Z4coef_table(1:end-4,:);  %Z4coef_table(1:end-4,:);   changed by AM 04142014
      end% for GE  machines  
          
      if Info.centerlistactivated == 75
         h_corrvect = Z4coef_table(2:end,1);
         klean_vect = Z4coef_table(2:end,2);
         km_vect = Z4coef_table(2:end,3);
      else
...
%      h_index = max(find(h_corrvect < thickness)); %Commented by SM on  09-29-2016
...
%     ydata_fat = fatref_vect';
    ...
    temproi=Image.image(roi_gen3.ymin:roi_gen3.ymin+roi_gen3.rows-1,roi_gen3.xmin:roi_gen3.xmin+roi_gen3.columns-1) - Analysis.Ibkg;
    temp_mean = mean(mean(temproi));
    figure(figuretodraw);
    funcBox(roi_gen3.xmin,roi_gen3.ymin,roi_gen3.xmin+roi_gen3.columns,roi_gen3.ymin+roi_gen3.rows,'b',3); 
    
    thicknessroi = zeros(size(temproi));
    thicknessroi(:,:) = thickness;
    %=thickness_ROIproj(roi_gen3.ymin:roi_gen3.ymin+roi_gen3.rows-1,roi_gen3.xmin:roi_gen3.xmin+roi_gen3.columns-1);
    thicknessroi_246cm = zeros(size(thicknessroi));
    thicknessroi_246cm(:,:) = thickness_246cm;
...
% % %     Lean_ref_ROI100 = (Lean_ref_ROI-Fat_ref_ROI)*0.25 + Lean_ref_ROI;
    ...
    Lean_ref_ROI_246cm = ph80_a2corr * thicknessroi_246cm.^2 + ph80_a1corr * thicknessroi_246cm + ph_offsetcorr;
    Fat_ref_ROI_246cm = ph_a2fat * thicknessroi_246cm.^2 +  ph_a1fat * thicknessroi_246cm + ph_offsetfat; 
% % %     Lean_ref_ROI100_real = (Lean_ref_ROI_real-Fat_ref_ROI_real)*0.25 + Lean_ref_ROI_real;
...
if MachineParams.comId > 0 %GEN 3 calibration
 ...
 %Added by Song 08-16-10, density conversion from wax-water reference to crisco reference (crisco_fat = -30% of DSP7) 
DensityImage = (temproi-Fat_ref_ROI)./(Lean_ref_ROI - Fat_ref_ROI)*80*crisco_coef;   %Then changed by Song 09-13-10, this conversion only applies to GEN3 calib                                         
if MachineParams.comId > 0
    DensityImage = refConvert(DensityImage);
end                                                                              %end of change
 DensityImage = funcclim(DensityImage,-100,200);  
% Inserted by Song 08-11-10, use a look-up table to correct densities Changed by Song 09-13-10, density corretion only applies to GEN3 calib
DensityImageBFCorr = DensityImage;
if MachineParams.comId > 0
    DensityImage = densCorr(DensityImage, thicknessroi, Database.Name, Analysis.GEN3commanal_id); %It was MachineParams.comId
end
  
% density calculated using 20, 40 and 60 246cm thicknesses known
   DensityImage_246cm = (temproi-Fat_ref_ROI_246cm)./(Lean_ref_ROI_246cm - Fat_ref_ROI_246cm)*80*crisco_coef; %kmnown thicknesses
   
   if MachineParams.comId > 0
    DensityImage_246cm = refConvert(DensityImage_246cm);
   end  
   DensityImage_246cm = funcclim(DensityImage_246cm,-100,200);
   DensityImage_246cmBFCorr = DensityImage_246cm;
   if MachineParams.comId > 0
    DensityImage_246cm = densCorr(DensityImage_246cm, thicknessroi_246cm, Database.Name, Analysis.GEN3commanal_id); %It was MachineParams.comId _246cm
   end
   
   SXAAnalysis.roiDensity=nansum(nansum(DensityImage.*thicknessroi))/sum(sum(thicknessroi));
   SXAAnalysis.roiVolumeProj = sum(sum(thicknessroi*(Analysis.Filmresolution*0.1)^2));
   SXAAnalysis.roiDenseVolume = SXAAnalysis.roiDensity*SXAAnalysis.roiVolumeProj/100;
   SXAAnalysis.roiDensity_246cm=nansum(nansum(DensityImage_246cm.*thicknessroi_246cm))/sum(sum(thicknessroi_246cm));
   SXAAnalysis.roiVolumeProj_246cm = sum(sum(thicknessroi_246cm*(Analysis.Filmresolution*0.1)^2));
   SXAAnalysis.roiDenseVolume_246cm = SXAAnalysis.roiDensity_246cm*SXAAnalysis.roiVolumeProj_246cm/100;
   SXAAnalysis.roiDensityBFCorr=nansum(nansum(DensityImageBFCorr.*thicknessroi))/sum(sum(thicknessroi));
   SXAAnalysis.roiDensity_246cmBFCorr=nansum(nansum(DensityImage_246cmBFCorr.*thicknessroi_246cm))/sum(sum(thicknessroi_246cm));
   a =1;
    %horizontal profile
...
SQLstatement = ['SELECT Top 1 * ', ...
                ...
                ' AND (version LIKE ''%Version8.0%'' or version LIKE ''%Version8.1%'' or version LIKE ''%Version8.3%'')',...
                'ORDER BY commonanalysis_id DESC'];
            
entryRead = mxDatabase(dbName, SQLstatement);
entryRead = entryRead(1,:);
...
SQLstatement = ['SELECT TOP 1 * FROM kTableGen3',...
               ...
               ' AND (machine_id = ', num2str(machID), ') ',...
               ' AND (version LIKE ''%Version7.1%'' OR version LIKE ''%Version8.0%'' OR version LIKE ''%Version8.1%'' OR version LIKE ''%Version8.3%'') AND (date_acquisition =',...
               ' (SELECT MAX(date_acquisition)  FROM kTableGen3 ',...
               ' WHERE (paddle_size lIKE ''', padSize, '''',')',...
               ' AND (machine_id = ', num2str(machID), ') ',...
               ' AND (version LIKE ''%Version7.1%'' OR version LIKE ''%Version8.0%'' OR version LIKE ''%Version8.1%'' OR version LIKE ''%Version8.3%'') ',...
...
s2 = regexp(acquisition_id_K, '  ', 'split');  % to prevent duplicate acq 
acquisition_id_K=cell2mat(s2(:,1));


SQLstatement1 = ['SELECT TOP 1 *  FROM kTableGen3',...
   ' WHERE (acquisition_id lIKE ''', acquisition_id_K, '''',')',...
    ' AND (QCWAX_status LIKE ''%True%'') ',...
    ' ORDER BY commonanalysis_id DESC'];  

kentry_Alt = mxDatabase(dbName, SQLstatement1);

Alt_kdate_num = datenum(kentry_Alt {1,4}, 'yyyymmdd');                

%modified 12-18-2018 to account for version change to Version8.3 sypksC
SQLstatement = ['SELECT MAX(date_acquisition), MIN(date_acquisition) FROM kTableGen3',...
               ' WHERE (paddle_size lIKE ''', padSize, '''',')',...
               ' AND (machine_id = ', num2str(machID), ') ',...
               ' AND (version LIKE ''%Version7.1%'' OR version LIKE ''%Version8.0%'' OR version LIKE ''%Version8.1%'' OR version LIKE ''%Version8.3%'') AND (QCWAX_status LIKE ''%True%'')' ];
...
% % % 
% % % Ktable = mxDatabase(Database.Name,['select * from kTableGen3 where machine_id= ',num2str(Info.machine_id), ' order by date_acquisition']);
% % %   sz_corr = size(Ktable);
% % %          
% % %             for i = 1:sz_corr(1)
% % %                 machine_id(i) = datenum( Ktable{i,1}, 'yyyymmdd');
% % %                 date_num(i) = datenum( Ktable{i,4}, 'yyyymmdd');
% % %                 paddle_type(i)  =  Ktable{i,5};
% % %                 km1(i) =Ktable{i,11};
% % %             end
...
% % %    b=mxDatabase(Database.Name,['select * from kTableGen3 where acquisition_id=',num2str(Info.AcquisitionKey)]);
