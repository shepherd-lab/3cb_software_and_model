function mask=funcloadimage(fname,Option)  
     global Result Info flag Image num_machine MachineParams Analysis 
...
%%%%%%%%%% extraction of room name as global variable Info.centerlistactivated %%%%%%%%%%%%%%%
 if Info.Database == false 
     ...
     Info.orientatin = info_dicom.PatientOrientation;
    ...
    % Analysis.Filmresolution = 0.14;      %only for Selenia machine
 elseif Info.DigitizerId ~= 3
     %for temporary
     %[pathstr,name,ext,versn] = fileparts(fname)
     %fmat_name = [pathstr,'\',name(1:end-3),'.mat']
     %load(fmat_name);
     %Info.Comments = info_dicom_blinded.ImageComments;
...
if Result.DXAProdigy ~= true;
    if Info.Database == false 
       RetrieveInDatabase('MACHINEPARAMETERS'); 
        if Result.DXASelenia == true | Analysis.PhantomID ~=9
            num_machine = Info.centerlistactivated;
            dark_counts = 0;
        else
            num_machine = Info.centerlistactivated;
           ...
           % dark_counts = 0;
            ...
            %num_machine = 8;
%         machine_counts = [ 65, 65, 64, 63, 65, 51, 60, 62];
       % machine_counts = [ 65, 65, 64, 63, 65, 51, 60, 0]; %%% the dark counts are put to zero.
       % dark_counts = machine_counts(num_machine);
    else
        RetrieveInDatabase('MACHINEPARAMETERS'); % for temporary
        num_machine = Info.centerlistactivated;
        dark_counts =MachineParams.dark_counts;
        %dark_counts = 0; %for temporary
    end
end
%}
%dark_counts = 0;
...
if ~exist('Option')
    Option='NONE';
        ...
        elseif ((fname(size(fname,2)-3:size(fname,2))=='.bmp')|fname(size(fname,2)-3:size(fname,2))=='.tif'|fname(size(fname,2)-3:size(fname,2))=='.png'|fname(size(fname,2)-3:size(fname,2))=='.gif')     %test image type
            if Info.DigitizerId >= 4 | flag.Selenia_image == true
               ...
               %if Info.kVp == 39
                  %dark_counts = 62;
                   Result.image=double(imread(fname)-dark_counts);%-64-70 -62   %%%%%%%%%%%%%%%%%%%%%%%%%%%%
                 ...
                 %Result.image = funcGradientGauss(I,3);
                  ...
                  if Result.DXASelenia == true
                       ...
                       switch Result.calibration_type
                          ...
                          case 'BreastCPMC'
                              Result.image2= log_convertDXA_cpmcDXAsm(Result.image,Info.mAs,Info.kVp,Info.Alfilter);
                           otherwise
                   ...
                   elseif Result.DXASelenia == false
                    %Result.image2= log_convert(Result.image);
                       ...
                       if Info.DigitizerId == 5 |  Info.DigitizerId == 6
                           Result.image2 = Result.image2 + 7000;
                       end
                       % Result.image2=Result.image;
                   end
