...
%       SMP_fft = sum(sum((u_v.^2).*abs(Zfft).^2))/(sum(sum(abs(Zfft).^2)));
    ...
    min_image = min(min(CurrentImage));
    CurrentImage = round((CurrentImage-min_image)/4) + round(min_image/4);
    max_image = max(max(CurrentImage));
    min_image = min(min(CurrentImage));
...
%     ma = histc(image_roi,bin);
   ...
   num_levels = max_image-min_image;
   s = zeros(1, num_levels);
   N = zeros(1, num_levels);
   p = zeros(1, num_levels);
   grv = zeros(1, num_levels);
