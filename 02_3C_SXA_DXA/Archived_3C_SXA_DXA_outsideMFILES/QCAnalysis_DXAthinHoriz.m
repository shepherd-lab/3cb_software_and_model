            ...
            for x_index=1:3
              if ~x_index %==1
                ...
                %if count < 9
                 ...
                 phantom_valuesLE(x_index,y_index) =mean(mean(Image.LE(ROI.Y1:ROI.Y2,ROI.X1:ROI.X2)));
                 phantom_valuesHE(x_index,y_index) =mean(mean(Image.HE(ROI.Y1:ROI.Y2,ROI.X1:ROI.X2)));
                 phantom_valuesRST(x_index,y_index) =mean(mean(Image.RST(ROI.Y1:ROI.Y2,ROI.X1:ROI.X2)));   
                   % else
...
%         density_vect = repmat([0 50 100],1,5);
        density_vect = repmat([100 61.54 23.08],1,5); %previous -30-100
        density_vect = repmat([80.8743 53.5519 26.2295],1,5);  %present  -48-135  
...
%         phantom_values2(:,1:2) = [thickness_vect',density_vect',];
        thickness_vect = sort(thickness_vect','descend');
        ...
        [fname,pname]=uiputfile('\\researchstg\aaData\Breast Studies\AL_SeleniaData\20110114\*.txt'); %20101223
