...
switch RequestedAction
    ...
    case 'compute'
        ...
        for index=1:Prodigy9stepsQAData.Draw.Compartments
            ...
            if flag.ShowMaterial == true %flag.ShowThickness == true
                 Prodigy9stepsQAData.Output{1,10+index} = mean(mean(Image.material(ROI.ymin:ROI.ymax,ROI.xmin:ROI.xmax)));  
                 Prodigy9stepsQAData.Output{2,10+index} = mean(mean(Image.thickness(ROI.ymin:ROI.ymax,ROI.xmin:ROI.xmax)));  
                 Prodigy9stepsQAData.Output{3,index} = mean(mean(Image.LE(ROI.ymin:ROI.ymax,ROI.xmin:ROI.xmax)));  
                 Prodigy9stepsQAData.Output{4,index} = mean(mean(Image.HE(ROI.ymin:ROI.ymax,ROI.xmin:ROI.xmax))); 
