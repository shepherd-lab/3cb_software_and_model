function sxa_acq_extraction()
  global database fname Info
    ...
    sz1 = size(ACQIDList);
    len = sz1(1);
    dir_towrite = '\\researchstg\aaData\Breast Studies\Masking\Masking_fullresolutions\MGH_priors\';
    dir_towrite_pres = '\\researchstg\aaData\Breast Studies\Masking\Masking_fullresolutions\MGH_priors\Presentation';
     key = 0;
     count = 0;
     count2=0;
    for index=1:len
...
%             Periphery_calculation; 
            ...
            XX1=round(OverSamplingN(image_pres,2));
            imwrite(uint16(XX1),pres_filename,'PNG');
