...
br_image = Image.LE.*~bw1;
%     BW1edge = edge(br_image,'Canny'); hold on;
% %    figure;imagesc(br_image);colormap(gray);hold on;
%     br_image = localcontrast(uint16(br_image),0.4,0.5);
   [centers, radii, metric] = imfindcircles(br_image,[300 400],'Sensitivity',0.999,'EdgeThreshold',0.1);
%    delete h;
...
%    h = viscircles(centers(1:n_cent,:),radii(1:n_cent)); %,'Method','twostage' 'ObjectPolarity','bright', 
   ...
   reg1 = regionprops(bw2,'centroid', 'Area');
   for i=1:length(reg1)
       area1(i) = reg1(i).Area;
   end
   [C,I] = max(area1);
...
if ~isempty(radii)   
   ...
   br_3 = br_image - br_2;
   [centers3, radii3, metric3] = imfindcircles(br_3,[300 400],'Sensitivity',0.998,'Method','twostage','EdgeThreshold',0.1);
...
%    spot_image = Image.LE.*breastspot_mask;
