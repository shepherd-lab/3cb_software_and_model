...
function CreateSXAReport_original(RequestedAction,param)
...
switch RequestedAction
    ...
    case 'SAVECLOSE'
            ...
            verFolder = Info.Version(end-1:end);
%             PowerPoint('saveclose','text',[startdir_report,'\AutomaticAnalysisReports7.0\', ...
%                        verFolder, '\AutomaticAnalysis',PowerPointReport.PatientIDbeginning,'_',num2str(param),'.ppt']);
%Modified by Song, 02-16-11
%save ppt to specific folder according to study site
            if (strcmpi(strtrim(Info.StudyID), 'cpmc') || strcmpi(strtrim(Info.StudyID), 'cpucsf'))
                siteFolder = 'CPMC';
                verFolder = Info.Version;
                saveDir = ['\\ming\aaData4\Breast Studies\', siteFolder, ...
                           '\AutomaticAnalysisReports\', verFolder];
                if exist(saveDir, 'dir') ~= 7   %7 means folder existing
                    mkdir(saveDir);
                end
                PowerPoint('saveclose', 'text', [saveDir '\AutomaticAnalysis', ...
                    PowerPointReport.PatientIDbeginning, '_', num2str(param), '.ppt']);
            
            elseif strcmpi(strtrim(Info.StudyID), 'avon')
                siteFolder = 'Shanghai';
                verFolder = Info.Version;
                saveDir = ['\\ming\aaData4\Breast Studies\', siteFolder, ...
                           '\AutomaticAnalysisReports\', verFolder];
                ...
                %default directory to save report
                siteFolder = strtrim(Info.StudyID);
                verFolder = Info.Version;
                saveDir = ['\\ming\aaData\Breast Studies\SXAanalysisReports\', ...
                            siteFolder, '\', verFolder];
