...
%     ROI = ROI2;  %commented because ROI2 is commmented
   ...
   Info.PeripheryComputed = false; 
    ...
    coef = 0.9;
      Analysis.params(1) = 0;    %/0.014); % 
   Analysis.params(2) = 0;    %/0.014);
   Analysis.params(3) = 30;
   Analysis.params(5) = 15;    %/0.014); % 
   Analysis.params(6) = 6;    %/0.014);
   Analysis.params(4) = 7.5;    %/0.014);
   MachineParams.x0_shift =0;
   MachineParams.y0_shift = 0;
   MachineParams.bucky_distance = 2;
   MachineParams.source_height = 66.4;
  ...
  Analysis.ry = 0;
    ...
    %commented and transfered to thickness_ROIcreation_v8;
...
% % %      end
     ...
     thickness_ROI = thickness_ROIcreation_v8();  %X_angle,Y_angle _v8
   ...
   % [outline_xcorr,outline_ycorr] = SkinDetection_corrected(Outline.x',Outline.y' ,midpoint,thickness_ROI);
    ...
    [outline_xcorr,outline_ycorr] = SkinDetection_correctedv2_v8(Outline.x',Outline.y');
...
%      figure; imagesc(inner_mask);colormap(gray);
     s=strel('disk',3);
     F=imerode(inner_mask,s);
     inner_mask=imdilate(F,s);
...
% % %          upper_corner = (1 - roipoly(ROI.image,upper_triangle(:,1),upper_triangle(:,2)));
% % %      else
% % %          upper_corner = (1 - roipoly(Image.image,upper_triangle(:,1),upper_triangle(:,2)));
% % %      end
% % %      %figure; imagesc(upper_corner); colormap(gray);
% % %      
% % %      %outline_xcorr,outline_ycorr
...
% % %      low_triangle = [[1, x_poly2, 1]', [ROI.rows, ROI.rows, low_one]'];
% % %      
% % %      if Info.ViewId == 4 | Info.ViewId == 5 %flag.Pectoral_MLOView
% % %          low_corner = (1 - roipoly(ROI.image,low_triangle(:,1),low_triangle(:,2)));
% % %          thickness_mapreal = zeros(size(ROI.image));
% % %      else
% % %          low_corner = (1 - roipoly(Image.image,low_triangle(:,1),low_triangle(:,2)));
% % %          thickness_mapreal = zeros(size(Image.image));
% % %      end
% % %       
% % %      two_corners = low_corner.*upper_corner;
...
% % %      figure;imagesc(diff);colormap(gray);
    ...
    % figure; imagesc(thickness_mapproj);colormap(gray); 
     ...
     %
     Image.Tmask3C = thickness_mapproj;
