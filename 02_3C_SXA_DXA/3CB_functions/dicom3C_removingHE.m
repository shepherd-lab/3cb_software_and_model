...
global ctrl
  %uigetdir
  parentdir = '\\researchstg\aaData\Breast Studies\3C_data\ConvertIntoPresentation';
  dirname_HE = '\\researchstg\aaData\Breast Studies\3C_data\ConvertIntoPresentation\DICOM_39kVp';
...
%    
    file_names = dir(parentdir); %returns the list of files in the specified directory
    sza = size(file_names);
    count = 0;
    count2 = 0;
    lenf = sza(1);
    ...
    warning off;
        ...
        for k = 3:lenf
            filename_read =  char(file_names(k).name);
            try
                ...
                kvp = info_dicom.KVP;
                if kvp == 39
                   movefile(fullfilename_read,dirname_HE);
