...
global Info Database QA ctrl Result Image
answer=false;
...
if strcmp(RequestedAction,'ROOT')
    if ~Info.QAWindow;
        ...
        windowsizey=0.7;ratio=0.5/windowsizey;  %allow to change the window height without change the aspect of the stuff inside
...
elseif strcmp(RequestedAction,'RETRIEVE')
    ...
    QAcodeFromDatabase=cell2mat(mxDatabase(Database.Name,SQLstatement));
    QAcodeFromDatabase=[QAcodeFromDatabase;0 0 0]; %to prevent empty QAcodeFromDatabase
    for index=1:QA.numbercode
        QAcodeNumber=cell2mat(QA.codeDescription(index,2));
        QA.value(QAcodeNumber)=(sum(QAcodeFromDatabase(:,3)==QAcodeNumber)>0);
    end
