...
%This function determine the phantom type. Typically the output is STEP
...
global Image Correction Error Analysis
   ...
   %figure; imagesc(current_image); colormap(gray);
    ...
    ExtractedImage=imcrop(current_image,rect);  %the region of SXA phantom
  ...
  %  imagesc(ExtractedImage); colormap(gray); 
   ...
   % imagesc(ExtractedImage); colormap(gray);
    ...
    [C,I]=max(histogram(1:round(size(histogram,2)/2)));
     ...
     bkgr = background_phantomdigital(ExtractedImage)+1000;
    
    Ibkg = I;% + imin;
    ExtractedImageNorm = uint8((ExtractedImage-bkgr) /(imax-Ibkg)*255);
