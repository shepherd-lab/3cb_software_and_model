function Periphery_calculation_phantom()
global X_angle ROI MachineParams Analysis Outline thickness_ROI Info thickness_mapproj  thickness_mapreal breast_Maskcorr PreciseOutline Error
 ...
 %try
   ...
   Info.PeripheryComputed = false; 
    ...
    %X_angle = Analysis.rx + MachineParams.rx_correction/1.2; % ??? for sign of angle
     ...
     thickness_ROI = thickness_ROIcreation(X_angle,Y_angle)+0.0; 
     %temproi=Image.image(ROI.ymin:ROI.ymin+ROI.rows-1,ROI.xmin:ROI.xmin+ROI.columns-1);
   %thickness = 2.027;
    %%%%%%%%%%%%%%%%%%%%%%  for flat phantom  %%%%%%%%%%%%%%%%%%%
    %{
    thickness = 6.039;%2.25; %4.012;%2.2 +  4.012; %2.2+2.027+0.96;thickness = 6.039;
    thickness_ROI = thickness*ones(size(ROI.image)); %ROI.image
     
    thickness_mapreal = thickness_ROI;
     %}
    thickness_mapreal = thickness_ROI;%+0.05;
