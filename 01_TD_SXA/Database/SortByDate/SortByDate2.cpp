...
{
	int s1,so1,SearchIndex,i,year,month,day,buflen,Index;
	long DateID,*DateNum;
	...
	if (nrhs==4) //put -2 for skipped analysis
	{
		...
		for (i=0;i<s3;i++)
		{
			...
			if (SearchIndex==so1)
                break;
            if 	((int) mxGetScalar(mxGetCell(prhs[0], SearchIndex))==Index)
	...
	for (i=0;i<s1;i++)
	{
        ...
        if (((int) mxGetScalar(mxGetCell(prhs[0], SearchIndex))==Index)&&(DateNum[SearchIndex]!=-2))
