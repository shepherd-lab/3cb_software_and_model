...
CalibrationPoints=load('\\ming.radiology.ucsf.edu\Working_Directory\Shepherd Shared Folders\Publications-Presentations\2008\Papers\3C Breast\CalibrationPoints20080826_fat_prtotein.txt');
...
Data_water=Data(:,5)-Data(:,2)-Data(:,1);
Result_water=Data(:,5)-Result(:,2)-Result(:,1);

dev_tfat = (sum((Data(:,1)-Result(:,1)).^2)./size(Data,1)).^0.5
dev_tprotein = (sum((Data(:,2)-Result(:,2)).^2)./size(Data,1)).^0.5
dev_twater = (sum((Data_protein-Result_protein).^2)./size(Data,1)).^0.5

fat_error=dev_tfat/max(Data(:,1))*100
protein_error=dev_tprotein/max(Data(:,2))*100
water_error=dev_twater/max(Data_water)*100

figure;plot(Result_water,'o');hold on;  %blue 'o' - calculated data 
