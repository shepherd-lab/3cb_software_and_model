function tophatFiltered = TOPHAT3(image, breast_mask)
...
breast_mask2 = imerode(breast_mask,SE60);
...
sz = size(original2);
temp = zeros(sz(1),sz(2)+200);
temp(:,201:end,:) = original2;
% temp(:,1:200) = repmat(original(:,1),1,200);
mirr_image = flipdim(original2(:,1:200),2);
temp(:,1:200) = mirr_image;
temp2 = imtophat(temp,se);
tophatFiltered = temp2(:,201:end);
...
ImgVector = sortrows(ImgVector);
        ...
        immax = prctile(sorted_array_B,99.99); %+1000;
