...
global startdir_report PathName FileName_list image_filename figuretodraw fname acquisitionkeyList
global xlsLineNum refDSPtoWAX debugMode SXAAnalysis temp_acqs duration SXAreport  Random_Acquisition  Nacquisition 
...
%noimage_flag = 0;
%Info.Database=true;
SXAreport =false;   %true for Powr Popint Reports
%profile on -history
cf = RequestedAction
number_acqs = 10000;  %matlab restart
Random_Acquisition=false; %true for choosing randomly acquisition_id
GEN3_MODE =true; % if it is true, error thickness is zero for GEN3 (Calibration)
...
switch RequestedAction
    ...
    case 'FILEOFLISTNAMES'
        ...
        if  Random_Acquisition==true;  % Added by AM 12172013
            ...
            [FileName_new,PathName] = uigetfile('\\researchstg\working-directory\Projects-Snap\aaSTUDIES\Breast Studies\Super SXA\Super SXA Analysis\Reading List New\UVM\2012\New 2012\','Select the acquisition list txt-file ');
...
% % %         end
        ...
        if acquisitionkeyList~=0   %0 if ncel button has not been pressed
            ...
            if size(acquisitionkeyList,1)==1    %1 patient mode
                ...
                %%Operator 20 is for automatic restarting Matlab, Is  is used in the startup.m file 
           ...
           if  date_acq_num>date_driveUVM1
            ...
            elseif strcmp(film_identifier(1:3),'ucs')  %for UCSF
                if strcmp(Info.patient_id(1:4),'3C01')
                      start_dir = ['\\researchstg\aaData\Breast Studies\3C_data\RO1_3Cimages\UCSF\',Info.patient_id(1:7),'\png_files'];
...
if Analysis.PhantomID == 9        %% Added by AM 03112014
