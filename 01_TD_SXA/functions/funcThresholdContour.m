...
global Threshold visu ROI  Image Error Outline Analysis ChestWallData figuretodraw Info Database BW
global BreastMask
%thcnt = 1
%tic
%try
   ...
   %a = ChestWallData.Valid;
      ...
      if Info.AutomaticThreshold==true;

        SQLstatement=['select * from manualthresholdanalysis where commonanalysis_id=',num2str(Threshold.CommonAnalysisKey),' order by commonanalysis_id DESC'];
        thresh_params = mxDatabase(Database.Name,SQLstatement);
        thresh_params=thresh_params(1,:);
        Threshold.value=cell2mat(thresh_params(3));
        Threshold.ThresholdResult = thresh_params(4);
        Analysis.Threshold_density = thresh_params(8);
   end;    
