function dicomread_directory_dialog()
global ctrl folderdatename

% folderdatename = 'T:\Breast Studies\AL_SeleniaData\try';
% uigetdir('Y:\Breast Studies\.'); % displays a dialog box enabling to choose the directory to convert

dirname_toread = folderdatename;
dirname_towritepngcut = '\png_files';
dirname_towritematcut = '\mat_files';

mkdir(dirname_toread,dirname_towritepngcut);  % create the two sub directories "png_files" and "mat_files"
mkdir(dirname_toread,dirname_towritematcut);
...
%         index_S = find(fullfilename_read=='S');
%         num = fullfilename_read(index_S(:,3):end-4);
        num = [];
    end
