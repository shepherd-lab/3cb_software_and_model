...
global ROI
global ctrl f0 Analysis  FreeForm  PreciseOutline Info Threshold Image data flag ManualEdge 
global Phantom Error figuretodraw axestodraw ChestWallData  Hist Result 
...
%try
    ...
    if get(ctrl.ShowBackGround,'value')
       set(ctrl.ShowBackGround,'value',false);
       back_h = figure('Name','Background Image');
       %set(figuretodraw, 'Visible','off');
       %axestodraw=axes;
       imagesc(Analysis.BackGround);colormap(gray);
       set(back_h,'position',[0 0 size(Image.image,2)/size(Image.image,1)*800 800]);
       figure(back_h);
    end  
...
% % %                error('Image size is equal to Background!');
% % %                stop;
% % %                
% % %             end;
        ...
        if (Analysis.Step>=3)&get(ctrl.CheckBreast,'Value')
            ...
            if Threshold.Computed
                try
                    ...
                    [indexreport,reportitem]=funcAddToReport(strcat('pixels:',num2str(Threshold.pixels),'  %:',num2str(Analysis.Threshold_density)),reportitem,indexreport); %Threshold.pixels/Analysis.ValidBreastSurface*100
                    [indexreport,reportitem]=funcAddToReport('',reportitem,indexreport);
                    [indexreport,reportitem]=funcAddToReport(strcat('threshold value:',num2str(Threshold.value)),reportitem,indexreport); %Threshold.pixels/Analysis.ValidBreastSurface*100
                    [indexreport,reportitem]=funcAddToReport('',reportitem,indexreport);
...
%                         plot((ROI.xmin+Threshold.boundary(:,2)-1)/Undersamplingfactor,(ROI.ymin+Threshold.boundary(:,1)-1)/Undersamplingfactor,'.r', 'LineWidth', 0.5,'markersize',0.5);
                         plot((ROI.xmin+Threshold.boundary_real(:,2))-1/Undersamplingfactor,(ROI.ymin+Threshold.boundary_real(:,1)-1)/Undersamplingfactor,'.y', 'LineWidth', 0.5,'markersize',0.5);
        ...
        if flag.Debug
            if (Analysis.Step>=7)
                if get(ctrl.SFLI,'Value')
                    ...
                    density_h = figure;imagesc(Analysis.SXADensityImageCrop);colormap(gray);colorbar;
                    set(density_h,'position',[0 0 size(Image.image,2)/size(Image.image,1)*800 800]);
                end
            end
        end
...
% % %         if flag.Density == true;; % for operator 27
% % %             
% % %             for k = 1:length(boundary_mask)
% % %                 boundary = boundary_mask{k};
% % %                 %     plot(boundary(:,2), boundary(:,1), 'r', 'LineWidth', 2)
% % %                 plot((ROI.xmin+boundary(:,2)-1)/Undersamplingfactor,(ROI.ymin+boundary(:,1)-1)/Undersamplingfactor,'.r', 'LineWidth', 0.5,'markersize',0.5);
% % % % %                 plot((ROI.xmin+Threshold.boundary(:,2)-1)/Undersamplingfactor,(ROI.ymin+Threshold.boundary(:,1)-1)/Undersamplingfactor,'.r', 'LineWidth', 0.5,'markersize',0.5);
% % %             end
% % %         end
       ...
       if(Analysis.Step>=2)&get(ctrl.CheckBreast,'Value')&get(ctrl.ShowAL,'value')
         if   Result.DXAProdigyBreastCalculated == true
            plot((Result.x75+ROI.xmin-1),( Result.y75+ROI.ymin-1),'linewidth',LINEPLOTSIZE-1,'color','r'); hold on;
            plot((Result.x50+ROI.xmin-1),( Result.y50+ROI.ymin-1),'linewidth',LINEPLOTSIZE-1,'color','y'); hold on;
            plot((Result.x25+ROI.xmin-1),( Result.y25+ROI.ymin-1),'linewidth',LINEPLOTSIZE-1,'color','g'); hold on;
        end
       end 
