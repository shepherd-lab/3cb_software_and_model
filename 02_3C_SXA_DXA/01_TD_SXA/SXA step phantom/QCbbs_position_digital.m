function [sort_coord] = QCbbs_position_digital()
   ...
   xk = 100;
    ...
    if Analysis.second_phantom == false
        rot_image = Image.image(1:y_height*k,x_width*k:end);
        rot_angle = 31.8;%rot_image2
        ycoord_shift = 0;
    else
        rot_image = Image.image(end-y_height*k:end,x_width*k:end);
        rot_angle =  -31.8;
        ycoord_shift = sz_image(1)-y_height*k;
    end
 ...
 %   imagesc(a1); colormap(gray);
  ...
  %{  
   ...
   %}
    ...
    %figure;
    %imagesc(bw2); colocroprmap(gray); hold off;
  %  BW = imread('text.png');
   %{   
...
% %     figure(h_init); hold on;
  % axes;
    ...
    % plot( stats2(i).Centroid(1),stats2(i).Centroid(2)-50 ,'.r','MarkerSize',7); hold on;
     ...
     xy_centroids(i,1:2) = [stats2(i).Centroid(1)+x_width*k,stats2(i).Centroid(2)-yk];
    end
   % xy_centroids(1:ln,1:2) = [stats2(1:ln).Centroid(1)+x_width,stats2(1:ln).Centroid(2)-50]; 
    coord = xy_centroids;
%Added for Avon GEN III, delete GEN3 BB mistakenlly detected as SXA
    while (ln > 9 && strcmpi(strtrim(Info.StudyID), 'avon'))
        [val, idx] = max(coord(:, 2));
        coord(idx, :) = [];
        ln = size(coord, 1);
    end
