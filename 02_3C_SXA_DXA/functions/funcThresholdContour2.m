...
% end
   ...
   % sz = size(boundary);
    ...
    Threshold.ComputedCalc=true; 
    thresh = Threshold.value
%     
%     figure;imagesc(BW);colormap(gray);
%      figure;imagesc(temproi.*~BW);colormap(gray);
     se1 = strel('disk',1);
     BW = imdilate(Image.ThresholdMask,se1);
%      maps.mask_calc2 = imresize(~BW,0.5);
%      temproi = imresize(temproi,0.5);
     maps.mask_calc2 = double(~BW);%.*maps.mask_calc2;
%      ReinitImage(temproi);
     
%      temp_image = temproi.*mask_calc;
% % %     hh = figure;imagesc(temproi.*mask_calc);colormap(gray);
%     figure;imagesc(Image.ThresholdMask );colormap(gray);
%      figure;imagesc(temproi.*mask_calc);colormap(gray);
%     
%     mask = roipoly(temproi,Threshold.boundary(:,2), Threshold.boundary(:,1));
