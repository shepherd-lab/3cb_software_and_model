function add3CB_UCSFDigiImages(patients)
...
load('O:\SRL\aaStudies\Breast Studies\3CB R01\Analysis Code\Matlab\temporary_UCSF\loading.mat')
...
%% pull "parameters" from dicomheader .mat file and add to "DICOMinfo" table
      parameter = [];
      field = [];
     if ~isempty(info_dicom_blinded)                  
         if ~isfield(info_dicom_blinded,'ManufacturerModelName')
             info_dicom_blinded.ManufacturerModelName = info_dicom_blinded.ManufacturersModelName;
         end
         
         if ~isfield(info_dicom_blinded,'ExposureInuAs')
             info_dicom_blinded.ExposureInuAs = info_dicom_blinded.ExposureinuAs;
         end
      else
...
%         end
        ...
        elseif strcmp(room_id,'H1KRHR849b9b58')
            field{6}='38'; %UCSF-ZM7
        elseif (strcmp(room_id,'H1KRHR8416ac98')||strcmp(room_id,'H1KRHR8416a80b') || strcmp(room_id,'H1KRHR83cdc72c'))
            field{6}='39';  %UCSF ZM-10
        elseif strcmp(room_id,'H1KRHR84e30148')||strcmp(room_id,'H1KRHR84e2c662')  ||strcmp(room_id,'H1KRHR84e2d824')   
            field{6}='40'; %UCSF_ZM6
