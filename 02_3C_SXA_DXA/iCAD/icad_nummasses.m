...
output = [];
 ...
 output_num{1,1}= 'patient_id';
 output_num{1,2}= 'num_masses';
 output_num{1,3}= 'num_clusters';
  output_num{1,4}= 'ipat';
for ipat=  1: num_pats   %[1]  %[110, 112, 128, 130, 131, 148, 7, 8, 28, 37, 48, 51] 
%     try
...
%     end % CC and MLO views
    count = count + 1
     output_num{count+1,1}= pat_cur;
     output_num{count+1,2}=num_masses
     output_num{count+1,3}= num_clusters;
     output_num{count+1,4}= ipat;
