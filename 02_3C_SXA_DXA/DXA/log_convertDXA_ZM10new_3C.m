function I = log_convertDXA_ZM10new(image,mAs, kVp, Alfilter)
global Image flag SXARefLE SXARefHE file Info
...
if comp(1)==true & comp(2)==true
      ...
      current_image3(:,1:1280) = image;
      image = current_image3;
%      image = image(193:1856,:); %for small paddle
end    
tomo = false;
...
%           if sz_le(1) < sz_FF(1)
%               
%               current_image2= current_image2(193:1855,1:1279); % crop image for small paddle images
%           else    sz_le(1) < 1800
%           current_image2= current_image2(193:1855,1:1279); % crop image for small paddle images
%           end
        ...
        current_image2(index_neg2) = 1; %flat field image
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if  kVp == 39  | kVp == 49 %Large paddle
        ...
        ROIref=image(Ymin:Ymax,Xmin:Xmax);
        signal=reshape(ROIref,1,prod(size(ROIref)));
        MEANref=nonan_mean(signal); % in the image
...
%         ROIref=image(YminSP:YmaxSP,XminSP:XmaxSP); % commented for Ge tomo  machine
%         signal=reshape(ROIref,1,prod(size(ROIref)));
%         MEANref=nonan_mean(signal);
...
%       flipdim(I0LE,2);I0LE=flipdim(I0LE,1); %both flips for MtZion
        ...
        sz_le = size(I0LE2);
        if sz_le(1) > 1800
            I0LE2= I0LE2(193:1855,1:1279); % crop image for small paddle images
        end
        index_neg1 = find(current_image1<=0);
        current_image1(index_neg1) = 1;
...
%         I0HE= imread(FF_filenameHE);
      ...
      I0HE= I0HE(193:1855,1:1279); % crop image for small paddle images
        ...
        %%% select ref ROI and calculate its mean value (in small paddle):
%         ROIref=image(YminSP:YmaxSP,XminSP:XmaxSP); %commented for Ge tomo
%         signal=reshape(ROIref,1,prod(size(ROIref)));
