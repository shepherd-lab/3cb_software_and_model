...
global data Database Info ctrl figuretodraw ReportText Error Image ROI  PhantomDetectionFailure   axestodraw f copyfigure_handle image
...
global DXAAnalysis SXAAnalysis startdir_report maps_CC mapsCC_noflip mapsML_noflip maps_ML Position Position1 pat10 patient_id Position_rad view
...
% % %          clear roicc xy flip_image;
        ...
        roicc =funcclim(roicc,0,8);
        %roicc  = medfilt2(roicc , [5 5]);
        if (strcmp(patient_id(3:7),'02019') | strcmp(patient_id(3:7),'02012'))
           foe=figure;imagesc(roicc, clims_water);colormap(jet);h=colorbar;set(h,'fontsize',34); hold on; 
        else
            foe=figure;imagesc(roicc);colormap(jet);h=colorbar;set(h,'fontsize',34); hold on; %,clims_water colormap(gray) caxis([0 3])
        end
        ...
        roicc =funcclim(roicc,0,8);
       %roicc  = medfilt2(roicc , [5 5]);
        if (strcmp(patient_id(3:7),'02019')  | strcmp(patient_id(3:7),'02012')) %strcmp(patient_id(3:4),'01')
           foe=figure;imagesc(roicc,clims_lipid);colormap(jet);h=colorbar;set(h,'fontsize',34); hold on; 
        else
            foe=figure;imagesc(roicc);colormap(jet);h=colorbar;set(h,'fontsize',34); hold on; % caxis([0 3])
        end
         plot(xy(:,1)-roi.xmin+roi.columnsk, xy(:,2)-roi.ymin+roi.rowsk, '-g');
        % PowerPoint('addtext','text',['Lipid ' ],'position',[Position(1)+0.55 Position(2)]);
        ...
        roi.rowsk = round(roi.rows*kc); roi.rows1k = round(roi.rows*(1+kc)); roi.columnsk = round(roi.columns*kc); roi.columns1k = round(roi.columns*(1+kc));
        roicc = flip_image(roi.ymin-roi.rowsk:roi.ymin+roi.rows1k-1,roi.xmin-roi.columnsk:roi.xmin+roi.columns1k-1);
        roicc =funcclim(roicc,-1,3);
        %roicc  = medfilt2(roicc , [5 5]);
        %foe=figure;imagesc(roicc);colormap(gray); h=colorbar;set(h,'fontsize',14); hold on; %,clims_water
        if (strcmp(patient_id(3:7),'02019') | strcmp(patient_id(3:7),'02012' )) % strcmp(patient_id(3:4),'01')
          foe=figure;imagesc(roicc,clims_prot);colormap(jet);h=colorbar;set(h,'fontsize',34); hold on; 
        else
          foe=figure;imagesc(roicc);colormap(jet); h=colorbar;set(h,'fontsize',34); hold on; %,
        end
        plot(xy(:,1)-roi.xmin+roi.columnsk, xy(:,2)-roi.ymin+roi.rowsk, '-g');
        % PowerPoint('addtext','text',['Lipid ' ],'position',[Position(1)+0.55 Position(2)]);
        ...
        roi.xmin = minx; roi.ymin = miny; roi.columns = (maxx - minx); roi.rows = (maxy-miny);
        roi.rowsk = round(roi.rows*km); 
        roi.rows1k = round(roi.rows*(1+km)); 
        roi.columnsk = round(roi.columns*km); 
        roi.columns1k = round(roi.columns*(1+km));
...
% %          PowerPoint('copypastefigure','position',[0.4 0.162 0.1 0.1],'LockAspectRatio',false);delete(foe);  
       ...
       %roicc  = medfilt2(roicc , [5 5]);
        %foe=figure;imagesc(roicc);colormap(gray); h=colorbar;set(h,'fontsize',14); hold on; %,clims_water
        if 0 % strcmp(patient_id(3:4),'01')
            foe=figure;imagesc(roicc,clims_water);colormap(jet);h=colorbar;set(h,'fontsize',34); hold on; 
        else            
             foe=figure;imagesc(roicc);colormap(jet);h=colorbar;set(h,'fontsize',34); hold on; %,clims_water
        end
        plot(xy(:,1)-roi.xmin+roi.columnsk, xy(:,2)-roi.ymin+roi.rowsk, '-g');
         %PowerPoint('addtext','text',['Lipid ' ],'position',[Position(1)+0.55 Position(2)]);
...
%          
        ...
        %%foe=figure;imagesc(roicc);colormap(gray); h=colorbar;set(h,'fontsize',14); hold on; %,clims_water
        if 0%strcmp(patient_id(3:4),'01')
            foe=figure;imagesc(roicc,clims_lipid);colormap(jet);h=colorbar;set(h,'fontsize',34); hold on; %,clims_water
        else             
            foe=figure;imagesc(roicc);colormap(jet);h=colorbar;set(h,'fontsize',34); hold on; 
        end
        plot(xy(:,1)-roi.xmin+roi.columnsk, xy(:,2)-roi.ymin+roi.rowsk, '-g');
...
% %           
% %      format bank;
      ind_pathol = find(strcmp('studyid',pat10(:,1)));
% %       PowerPoint('AddSlide');
...
% %           PowerPoint('addtext','text',[''],'position',[0.02 Position1(2)+0.04],'carriage',0.5);  
       ...
       PowerPoint('addtext','text',['Results:'],'position',[Position(1) Position1(2)]); PowerPoint('addtext','text',[''],'position',[Position(1)+0.4 Position1(2)],'carriage',1.0);
        ...
        PowerPoint('addtext','text',['MLO view'],'bold',true,'fontsize',1.0,'position',[Position(1)+0.6 Position1(2)],'carriage',1.0); 
        Position1=PowerPoint('gettextposition');
       % sz = size(results_3CB);
        if ~isempty(maps_CC.results) %& sz(1)>= 1
            PowerPoint('addtext','text',['Lesion ROI water/lipid/protein, cm:'],'position',[Position(1) Position1(2)]); PowerPoint('addtext','text',[ sprintf('%.2f',cc.ROIWmed),'/', sprintf('%.2f',cc.ROILmed),'/', sprintf('%.2f',cc.ROIPmed)],'position',[Position(1)+0.4 Position1(2)]); % /total  ,'/',num2str(results_3CB(1,4))
            PowerPoint('addtext','text',[ sprintf('%.2f',ml.ROIWmed),'/', sprintf('%.2f',ml.ROILmed),'/', sprintf('%.2f',ml.ROIPmed)],'position',[Position(1)+0.6 Position1(2)],'carriage',1.0); %,'/',num2str(results_3CB(1,4))
            Position1=PowerPoint('gettextposition');
        else
         ...
         if ~isempty(maps_CC.results) %& sz(1)>= 1
            PowerPoint('addtext','text',['Periphery water/lipid/protein, cm:'],'position',[Position(1) Position1(2)]); PowerPoint('addtext','text',[sprintf('%.2f',cc.bkgrW1med),'/',sprintf('%.2f',cc.bkgrL1med),'/',sprintf('%.2f',cc.bkgrP1med)],'position',[Position(1)+0.4 Position1(2)]); % /total  ,'/',num2str(results_3CB(1,4))
            PowerPoint('addtext','text',[sprintf('%.2f',ml.bkgrW1med),'/',sprintf('%.2f',ml.bkgrL1med),'/',sprintf('%.2f',ml.bkgrP1med)],'position',[Position(1)+0.6 Position1(2)],'carriage',1.0); %,'/',num2str(results_3CB(1,4))
            Position1=PowerPoint('gettextposition');
        else
            PowerPoint('addtext','text',['Breast water/lipid/protein, cm:'],'position',[Position(1) Position1(2)]); PowerPoint('addtext','text',' -/-/-/-','position',[Position(1)+0.4 Position1(2)],'carriage',1.0); %/total
            Position1=PowerPoint('gettextposition');
        ...
        if ~isempty(maps_CC.results) & strcmp(patient_id(3:4),'01')
            PowerPoint('addtext','text',['Breast water/lipid/protein, cm:'],'position',[Position(1) Position1(2)]); PowerPoint('addtext','text',[sprintf('%.2f',cc.breastWmed),'/',sprintf('%.2f',cc.breastLmed),'/',sprintf('%.2f',cc.breastPmed)],'position',[Position(1)+0.4 Position1(2)]); % /total  ,'/',num2str(results_3CB(1,4))
            PowerPoint('addtext','text',[sprintf('%.2f',ml.breastWmed),'/',sprintf('%.2f',ml.breastLmed),'/',sprintf('%.2f',ml.breastPmed)],'position',[Position(1)+0.6 Position1(2)],'carriage',1.0); %,'/',num2str(results_3CB(1,4))
            Position1=PowerPoint('gettextposition');
        else
            PowerPoint('addtext','text',['Breast water/lipid/protein, cm:'],'position',[Position(1) Position1(2)]); PowerPoint('addtext','text',' -/-/-/-','position',[Position(1)+0.4 Position1(2)],'carriage',1.0); %/total
            Position1=PowerPoint('gettextposition');
...
% %         end
    ...
    case 'MOFFITTIMAGES'   
         ...
         Position=PowerPoint('gettextposition');
         clear maps;
         load(['\\researchstg\aaData\Breast Studies\3C_data\RO1_3Cimages\Moffitt\3CB_maps_wo3CBorig\', patient_id,'_',view,'.mat']);
         foe=figure;imagesc(maps.LEatten);colormap(gray);hold on;
         sz = size(maps.lesion);
         len = sz(2)
         for i=1:len
         ...
         copyfigure_handle = foe;
         PowerPoint('copypastefigure','position',[0.1 0.03 0.20 0.20],'LockAspectRatio',false);delete(foe);
         foe=figure;imagesc(maps.LEPres);colormap(gray);hold on;
         for i=1:len
             plot(maps.lesion(i).xy(:,1),maps.lesion(i).xy(:,2),'b-')
         end
         copyfigure_handle = foe;
         PowerPoint('copypastefigure','position',[0.5 0.03 0.20 0.20],'LockAspectRatio',false);delete(foe);
         foe=figure;imagesc(maps.LEPresOrig);colormap(gray);hold on;
         for i=1:len
             plot(maps.lesion(i).xy_orig(:,1),maps.lesion(i).xy_orig(:,2),'b-')
         end
         hold off;  copyfigure_handle = foe;
         PowerPoint('copypastefigure','position',[0.1 0.25 0.20 0.20],'LockAspectRatio',false);delete(foe);
         foe=figure;imagesc(maps.LERawOrig);colormap(gray);hold on;
         for i=1:len
             plot(maps.lesion(i).xy_orig(:,1),maps.lesion(i).xy_orig(:,2),'b-')
         end
         hold off;  copyfigure_handle = foe;
         PowerPoint('copypastefigure','position',[0.5 0.25 0.20 0.20],'LockAspectRatio',false);delete(foe);
         annot_name = ['\\researchstg\aaData\Breast Studies\3C_data\RO1_3Cimages\Moffitt\Moffitt_Annotation_images\Annotations\',patient_id,'_',view,'_annotation.mat']
         foe=figure;imagesc(maps.ROI_orig.BackGround);colormap(gray);
         hold off;  copyfigure_handle = foe;
         PowerPoint('copypastefigure','position',[0.1 0.5 0.20 0.20],'LockAspectRatio',false);delete(foe);
         
         foe=figure;imagesc(~maps.ROI_orig.BackGround.*double(maps.LERawOrig));colormap(gray);
         hold off;  copyfigure_handle = foe;
         PowerPoint('copypastefigure','position',[0.5 0.5 0.20 0.20],'LockAspectRatio',false);delete(foe);
         clear maps;
         load(annot_name);
         foe = figure; imagesc(image);colormap(gray);
         hold on;
         for i=1:len
             plot(FreeForm.FreeFormCluster(i).face(:,1),FreeForm.FreeFormCluster(i).face(:,2),'-b');
         end
         hold off;  copyfigure_handle = foe;
         PowerPoint('copypastefigure','position',[0.1 0.75 0.25 0.25],'LockAspectRatio',false);delete(foe);      
     ...
     case 'UCSFMAGES'   
         ...
         Position=PowerPoint('gettextposition');
         clear maps;
         load(['\\researchstg\aaData\Breast Studies\3C_data\RO1_3Cimages\UCSF\3CB_maps_wo3CBorig\', patient_id,'_',view,'.mat']);
         foe=figure;imagesc(maps.LEatten);colormap(gray);hold on;
         sz = size(maps.lesion);
         len = sz(2)
         ...
         hold off;  copyfigure_handle = foe;
         PowerPoint('copypastefigure','position',[0.5 0.25 0.20 0.20],'LockAspectRatio',false);delete(foe);
         annot_name = ['\\researchstg\aaData\Breast Studies\3C_data\RO1_3Cimages\UCSF\UCSF_Annotation_images\Annotations\',patient_id,'_',view,'_annotation.mat']
         foe=figure;imagesc(maps.ROI_orig.BackGround);colormap(gray);
         hold off;  copyfigure_handle = foe;
         PowerPoint('copypastefigure','position',[0.1 0.5 0.20 0.20],'LockAspectRatio',false);delete(foe);
    ...
    case 'iCAD'
         ...
         PowerPoint('copypastefigure','position',[0.5 0.03 0.45 0.45],'LockAspectRatio',false);delete(foe);
         foe=figure;imagesc(maps.LEPresOrig);colormap(gray);hold on;
         for i=1:len
             plot(maps.lesion(i).xy_orig(:,1),maps.lesion(i).xy_orig(:,2),'b-')
         end
