...
ND = size(D);
NT = size(T);
dev_first = (sum((D-DR).^2)./ND(1)).^0.5
dev_second = (sum((T-TR).^2)./NT(1)).^0.5

density_error=dev_first/max(D)*100
thickness_error=dev_second/max(T)*100
