function thickness_ROI = thickness_ROIcreationOneplain(X_angle,Y_angle)
  ...
  % middle = (ROI.ymax-ROI.ymin)/2;
   ...
   y_linspace = (1:ROI.rows) + ROI.ymin ;
    for i = 1:x_ROI %x_ROI %xmax_pixels
       ...
       Y1 = (half - y_linspace)*resolution ; % + y0_shift
      ...
      % Y1 =  (ymax_pixels/2+y0_shift - ROI.ymin)*resolution - y_linspace(1:middle);
       ...
       thickness_ROI(:,i) = plane_coef(1)* X1 + plane_coef(2)*Y1 + plane_coef(3);
   ...
   %}
    thickness_ROI = thickness_ROI - MachineParams.bucky_distance - 0.23;
    a = 1;
