...
load('\\researchstg\Working_Directory\Projects-Snap\aaSTUDIES\Breast Studies\BCSC\Analysis Code\Matlab\Versions\Development\3C SXA DXA\Moffitt3Cloading.mat');
...
for pn = pat_min:pat_max
    ...
    while (i<=length(files) & ii < 1)
        if  (~isempty(strfind(lower(files(i).name),'lecc')) & isempty(strfind(lower(files(i).name),'orig')) & ~isempty(strfind(lower(files(i).name),'raw')) )                                   %~isempty(strfind(lower(files(i).name),'lecc'))            
