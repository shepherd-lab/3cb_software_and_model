...
if size_version(1) > 0 | Info.DigitizerId ~= 4
...
%% ROI
        ...
        eval(CallBack);
         multiWaitbar( 'Automatic SXA Analysis Progress',  1/4, 'Color', [0.2 0.9 0.3] );

%% Skin detection
        AutomaticAnalysis.Step=5;
        CallBack=get(ctrl.SkinDetection,'callback');  %press on SkinDection button
        eval(CallBack);
         multiWaitbar( 'Automatic SXA Analysis Progress',  2/4, 'Color', [0.2 0.9 0.3] );
...
%% Read the Tag
  ...
  %% phantom detection
    ...
    eval(CallBack);
     multiWaitbar( 'Automatic SXA Analysis Progress',  3/4, 'Color', [0.2 0.9 0.3] );
    if ~Error.PhantomDetection
        ...
        if Analysis.PhantomID == 7 | Analysis.PhantomID == 8
            X_right = ROI.xmax + 50;
        else
