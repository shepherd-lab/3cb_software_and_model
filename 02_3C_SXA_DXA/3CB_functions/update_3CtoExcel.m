...
%     rootdir=[pwd,'\'];
run_number = maps.results.Analysis_run;
if ~isempty(findstr(center,'ucsf'))
    dir_run1 = ['\\researchstg\aaData\Breast Studies\3C_data\RO1_3Cimages\UCSF\Results\Auto_3C_analysis_runs\Run',num2str(run_number),'\'];
    fileName = ['UCSF_run',num2str(run_number),'.xlsx'];
elseif ~isempty(findstr(center,'moff'))
    dir_run1 = ['\\researchstg\aaData\Breast Studies\3C_data\RO1_3Cimages\MOffitt\Results\Auto_3C_analysis_runs\Run',num2str(run_number),'\'];
...
%     catch
%       msgbox('Can not find UCSF_run1.xls file in the root directory!','3CB output excel file problem','error');
%     end
...
%    else
    ...
    xlsLineNum = rowNum; 
    if xlsLineNum == 1 | isempty(xlsLineNum)
       failed = raw(xlsLineNum,1)
       stop;
    else
    xlswrite(filePathName,results3C , '3CBanalysis', ['A', num2str(xlsLineNum)]);
    end
