function  Z4ComputeBreastDensity(Innerline,ROI, Image)
    global Info bb Analysis params roi_values Error h_slope Z4coef_table ROI MachineParams thickness_ROI
    Error.DENSITY=false;
   ...
   tx = round(Analysis.params(5));    %/0.014); % 
   ty = round(Analysis.params(6));    %/0.014);
   tz = round(Analysis.params(4));    %/0.014);
   ...
   x_linspace = 1:x_ROI;
   y_linspace = ((1:y_ROI)')*resolution;  %cm
   
   X = repmat(x_linspace', y_ROI,1);
   Y = ones(x_ROI,1);
   for i = 2:y_ROI
       Y = [Y;i*ones(x_ROI,1)];
   end    
   Z = ones(x_ROI*y_ROI, 1)*tz;
   one_colm = ones(x_ROI*y_ROI,1);
