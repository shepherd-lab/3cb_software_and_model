...
global Info Image ROI  Threshold  Analysis flag chestwall_Mask BreastMask Database boundary_mask  
...
Analysis.DensityPercentageMD=nonan_sum(nonan_sum(DensityImageSkin.*MaskROIproj.*thickness_mapprojCrop.*BW))/sum(sum(MaskROIproj.*thickness_mapprojCrop.*BW));
...
Analysis.DensityPercentageAutoTR =nonan_sum(nonan_sum(DensityImageSkin.*MaskROIproj.*thickness_mapprojCrop.*I2))/sum(sum(MaskROIproj.*thickness_mapprojCrop.*I2)); 
...
FD.OLP_OLV_HD=[Analysis.OLP,Analysis.OLV,Analysis.hd,Analysis.VolumeMD_Real,Analysis.DensityPercentageMD,Analysis.VolumeMD_AutoTR,Analysis.DensityPercentageAutoTR];
end
