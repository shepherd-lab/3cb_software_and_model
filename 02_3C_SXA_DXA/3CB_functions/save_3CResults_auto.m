...
%added 05252018 sypks to redirect saved output
if flag.OUTPUT==1
    fname_results = ['O:\SRL\aaData\Breast Studies\3C_data\RO1_3Cimages\temporary_UCSF\3cbMaps',fname];
else
    fname_results = [pathstr,fname];
end
if flag.spot_paddle == true
    if flag.rect_spot_paddle == false %modified to include this if statement to account for rectangle spot paddle 4/26/18 sypks
        breastspot_mask  = circle_spotpaddle();
        im_size = size(breastspot_mask);
        x_max = ROI.xmin+ROI.columns;
        y_max = ROI.ymin+ROI.rows;
        if x_max > im_size(2)
            ROI.xmin = im_size(2) - ROI.columns;
        end
        if y_max > im_size(1)
            ROI.ymin = im_size(1) - ROI.rows;
        end
    else
        X = mean(FreeForm.FreeFormCluster.face(:,1));
        Y = mean(FreeForm.FreeFormCluster.face(:,2));
        breastspot_mask = createCirclesMask(Image.LE,[X,Y],350);
    end
...
%Modified FreeForm.FreeFormCluster(1,i).face(:,) to accept
%FreeForm.FreeFormSelect and selection in data cluster by sypks 4.3.2018

%original before modification 4.3.2018
%{
...
%modified 4.3.2018 sypks
for i = 1:length(FreeForm.FreeFormCluster(1,FreeForm.FreeFormSelect,:))
    maps.lesion(1,i).xy(:,1) = FreeForm.FreeFormCluster(1,FreeForm.FreeFormSelect,i).face(:,1)- ROI.xmin;      %resize leison region ROI scale
    maps.lesion(1,i).xy(:,2) = FreeForm.FreeFormCluster(1,FreeForm.FreeFormSelect,i).face(:,2)- ROI.ymin;      %in FreeForm.FreeFormCluster.face to ROI.xmin..
    lesion_ROI2(:,:,i) = double(roipoly(maps.LEPres,maps.lesion(1,i).xy(:,1),maps.lesion(1,i).xy(:,2))).*MaskROI_breast;
end
...
for i=1:length(FreeForm.FreeFormCluster(1,FreeForm.FreeFormSelect,:))      %modified sypks 4.3.2018
    ...
    %  boundary = bwtraceboundary(inner_mask,[ymin,1],'E'); not used
    % % bl = cell2mat(bwboundaries(lesion_ROI));
    % % bth_1 = cell2mat(bwboundaries(distbw_thresh1));
    % % bth_2 = cell2mat(bwboundaries(distbw_thresh2));
    % % bth_3 = cell2mat(bwboundaries(distbw_thresh3));
    % % ind = find(maps.breast_mask==0);
    % % maps.lipid(ind) = 3;
    % % % figure;imagesc(maps.lipid);colormap(gray);colorbar('FontSize',10); hold on; plot(bl(:,2),bl(:,1),'-b',bth_1(:,2),bth_1(:,1),'-r',bth_2(:,2),bth_2(:,1),'-g',bth_3(:,2),bth_3(:,1),'-magenta');
    % % figure;imagesc(maps.LEPres);colormap(gray);colorbar('FontSize',10); hold on; plot(bl(:,2),bl(:,1),'-b',bth_1(:,2),bth_1(:,1),'-r',bth_2(:,2),bth_2(:,1),'-g',bth_3(:,2),bth_3(:,1),'-magenta');
    
    %% lesion ROI
    %   filter_ROILzero = maps.lipid.*lesion_ROI;
    ...
    % to handle NaN for entropy function sypks 07262018
    % filter_ROIW(isnan(filter_ROIW)) = 0;
    ...
    % correction 08062018 by sypks
    % source of error regarding identical ROILmean and breastLmean has been
    % traced to incorrect call to ~isnan(filter_ROIL) instead of ~isnan(filter_breastL)
    filter_breastL = maps.lipid.*maps.breast_mask;
    %filter_breastLvect = filter_breastL(~isnan(filter_ROIL)); original
    filter_breastLvect = filter_breastL(~isnan(filter_breastL)); %correction
    ...
    %temporary fix as patient 3C01018_ML MaskROI_breast has a small error
    %associated with it. Created if statement to handle by choosing larger area
    %given
    exLIST = ['3C01018_ML',];
    if strcmp(patient_ID,exLIST)~=0
        maps.results.breast_area = max(vertcat(s.Area))*0.014*0.014;   %added 4.6.2018 sypks
    else
        maps.results.breast_area = s.Area*0.014*0.014; %original
    end
    %  figure; imagesc(maps.breast_mask); colormap(gray);
    ...
    %%
    % UCSF original code directory save (probably)
    %{
 ...
 results_dir = '\\researchstg\aaData\Breast Studies\3C_data\RO1_3Cimages\MOffitt\Results\Auto_3C_analysis_runs\Run2\';
    ...
    %new temp location for results 08-14-2018 sypks
    if (flag.OUTPUT)
        results_dir = 'O:\SRL\aaData\Breast Studies\3C_data\RO1_3Cimages\RadPub2018\Run3';
    else
        results_dir = 'O:\SRL\aaData\Breast Studies\3C_data\RO1_3Cimages\results_temp_03292018';
    end
    %changed result directory PATH 3.29.2018 (UHM) temporarily removed while
    %backup is being done
    %{
   if ~isempty(findstr(center,'ucsf'))
 results_dir = 'O:\SRL\aaData\Breast Studies\3C_data\RO1_3Cimages\UCSF\Results\Auto_3C_analysis_runs\Run2\';
 elseif ~isempty(findstr(center,'ucsf_pipot'))
    results_dir = 'O:\SRL\aaData\Breast Studies\3C_data\RO1_3Cimages\UCSF_pilot\Results\Auto_3C_analysis_runs\Run2\';
 else
 results_dir = 'O:\SRL\aaData\Breast Studies\3C_data\RO1_3Cimages\MOffitt\Results\Auto_3C_analysis_runs\Run2\';
 
  end
    %}
    
    %{
     OLD UCSF RESULTS DIRECTORY PATHS
 %~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  if ~isempty(findstr(center,'ucsf'))
 results_dir = '\\researchstg\aaData\Breast Studies\3C_data\RO1_3Cimages\UCSF\Results\Auto_3C_analysis_runs\Run2\';
 elseif ~isempty(findstr(center,'ucsf_pipot'))
