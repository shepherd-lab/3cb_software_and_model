...
global ctrl
  parentdir = uigetdir('\\researchstg\aaData\Breast Studies\3C_data\RO1_3Cimages\Moffitt\');
% %    bd = dir('\\researchstg\aaData\Breast Studies\3C_data\RO1_3Cimages\Moffitt');
% % %    for i=[70:70]
% %  parentdir = ['\\researchstg\aaData\Breast Studies\3C_data\RO1_3Cimages\Moffitt\3C020',num2str(i,'%03.0f')];
        ...
        if ~isdir(name2)    % ( (strcmp(lower(ext), '.dcm') | ~isempty(ext) ) & ~isdir(name)  )
            ...
            elseif strfind(mode, 'MANUAL')
                if (kvp==25 & mas>49 && mas<51)% & paddle(1:4) == uint8([78;111;110;101])
                    file_short = 'DC';
                elseif ((kvp == 39 | kvp == 38) & (paddle(1:4) == uint8([50;52;99;109]) | paddle(1:4) == uint8([49;56;32;99])) ) %;109
                    file_short = 'CPHE';
                    
                elseif ((kvp~= 39 | kvp ~= 38) & (paddle(1:4) == uint8([50;52;99;109]) | paddle(1:4) == uint8([49;56;32;99])) & thickness >= 76 ) %& (thickness > 84 & time > 735500)
                    file_short = ['CPLE',num2str(kvp)];
                elseif ((kvp~= 39 | kvp ~= 38) & (paddle(1:4) == uint8([50;52;99;109]) | paddle(1:4) == uint8([49;56;32;99])) & ((thickness < 76))) % & time > 735500) | (thickness < 70 & time < 735500)
                    file_short = ['GEN3','_',num2str(kvp),'_',num2str(force)];
...
for k = 3:lenf
    ...
    if ~isdir(name2)    % ( (strcmp(lower(ext), '.dcm') | ~isempty(ext) ) & ~isdir(name)  )
        ...
        elseif strfind(mode, 'MANUAL')
            ...
            elseif ((kvp~= 39 | kvp ~= 38) & (paddle(1:4) == uint8([50;52;99;109]) | paddle(1:4) == uint8([49;56;32;99])) & thickness >= 77.5 ) %& (thickness > 84 & time > 735500)
                file_short = ['CPLE',num2str(kvp)];
            elseif ((kvp~= 39 | kvp ~= 38) & (paddle(1:4) == uint8([50;52;99;109]) | paddle(1:4) == uint8([49;56;32;99])) & ((thickness < 77.5))) % & time > 735500) | (thickness < 70 & time < 735500)
                file_short = ['GEN3','_',num2str(kvp),'_',num2str(force)];
                % %             elseif ((kvp~= 39 | kvp ~= 38) & (paddle(1:4) == uint8([50;52;99;109]) | paddle(1:4) == uint8([49;56;32;99])) & (thickness >70)) % & time < 735500
                % %                          file_short = ['CPLE',num2str(kvp)];
