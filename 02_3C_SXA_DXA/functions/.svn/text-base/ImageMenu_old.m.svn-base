...
global Analysis ctrl Image Result Info
...
switch RequestedAction
    ...
    case 'Label Removal'   
        ...
        Image.image(p1(2):p1(2)+offset(2),p1(1):p1(1)+offset(1)) = 9500;
        figure;imagesc(Image.image);colormap(gray);
        imwrite(uint16(Image.image),Info.fname,'PNG');
        Analysis.BackGroundComputed==false
...
%                        LocalMessage=strcat('Mean LE: ',num2str(mean(mean(tempimage2))),'; Mean HE:  ',num2str(mean(mean(tempimage))),';  Mean Material: ',num2str(mean(mean(tempimage4))),';  Mean Thickness:',num2str(mean(mean(tempimage5))),';  Mean ThirdComp:',num2str(mean(mean(tempimage6)))); %-50
