...
figure;surfl(Y,X,image(1:end,1:end)); camlight left; shading interp; 

ylabel('cm','fontsize',14);xlabel('cm','fontsize',14);zlabel('cm','fontsize',14);

% % for protein
% title('Protein thickness', 'Fontsize',14);
% axis([0 4 0 4 0 1]);
% for fat
title('Fat thickness', 'Fontsize',14);
axis([0 4.5 0 4.5 0 4.5]);
colormap(bone); % for fat
colormap(pink);
 view([-50 40]);grid('on');
