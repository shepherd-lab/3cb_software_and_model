...
%i<=length(files)
    while i<=length(files)
        try
            ...
            movefile(source1,destination);
            copyfile(source2,destination);
        catch
            lasterr
            if errorCount==3
                !C:\MATLAB7\bin\win32\matlab.exe -nosplash -r "cd 'Z:\aaSTUDIES\Breast Studies\CPMC\Analysis Code\Matlab\SXAVersion6.4_March12\'"&
                exit;
            end
