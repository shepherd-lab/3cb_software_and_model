function thickness_mapproj = projection_conversionTILT()
    global thickness_ROI MachineParams Analysis ROI Image
    %y0 = mid_point;
   % S = 66.3;
    %res = 0.014;
    ...
    corr_coeff = 1;
    thickness_mapreal = thickness_ROI - h; %for 0.44cm plate - 0.234;%2.237;  %2.65 for 0.44 plate and N11;
       % temproi=Image.image(ROI.ymin:ROI.ymin+ROI.rows-1,ROI.xmin:ROI.xmin+ROI.columns-1);
       sz = size(Image.OriginalImage);  
       xmax_pixels = sz(2);
       ymax_pixels = sz(1);
       y0 = ymax_pixels / 2;
       temproi = zeros(sz); %temproi = zeros(ROI.rows, ROI.columns);
       ...
       % figure; imagesc(thickness_ROIupper);colormap(gray);
        ...
        tic
        for i = 1:xmax_pixels %ROI.columns
