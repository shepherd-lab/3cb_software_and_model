...
function ImageMenu(RequestedAction)
...
switch RequestedAction    
    case 'BeginDrawing'  %0
        ...
        if sum(flipdim(point1(1,1:2),2)>size(Image.image))|sum(flipdim(point1(1,1:2),2)<0)
            ...
            set(f0.handle,'WindowButtonUpFcn','ImageMenu(''EndDrawing'')');
            set(f0.handle,'WindowButtonMotionFcn','ImageMenu(''MoveDraw'')','Interruptible','off');
        end
    case 'MoveDraw'
    ...
    case 'ModifyFreeForm' %2
        ...
        if FreeForm.FreeFormnumber>0
            ...
            if sum(flipdim(point1(1,1:2),2)>size(Image.image))|sum(flipdim(point1(1,1:2),2)<0)
                ...
                set(f0.handle,'WindowButtonMotionFcn','ImageMenu(''MoveModifyFreeform'')','Interruptible','off');    
            end
        else
            funcActivateDeactivateButton; %defreeze
        end
