function add3C_TESTDigiImages(patients)
...
% add all 3cb files and folders to path
rootdir=['O:\SRL\aaStudies\Breast Studies\3CB R01\Analysis Code\Matlab\Developers\sypks\github\shepherd-lab\3cb','\'];
addpath(rootdir);
...
addpath(['O:\SRL\aaStudies\Breast Studies\3CB R01\Analysis Code\Matlab\Developers\sypks\github\shepherd-lab\3cb\01_TD_SXA\Compiler']);
addpath(['O:\SRL\aaStudies\Breast Studies\3CB R01\Analysis Code\Matlab\Developers\sypks\github\shepherd-lab\3cb\01_TD_SXA\Database']);
...
load('O:\SRL\aaStudies\Breast Studies\3CB R01\Analysis Code\Matlab\Developers\sypks\github\shepherd-lab\3cb\00_initialization\database_loading\test3C_loading.mat');
...
i=1;
pat_dir = ['\3C03',num2str(pn, '%03i')]; 
patientID = ['3C03',num2str(pn, '%03i')]; ;
...
while i<=length(files)
    ...
    if ~isfield(info_dicom,'Format')
        info_dicom = info_dicom_blinded;    %05242018 workaround since some are named info_dicom_blinded sypks
    end
...
% % %         if isfield(info_dicom_blinded,'ExposureInuAs')
...
% % %         end
        ...
        %info_dicom_blinded.ExposureInuAs
...
%             field{6}='1';
%         end
     if ~isfield(info_dicom_blinded,'DeviceSerialNumber')
         room_id = [];
     else
        room_id=info_dicom_blinded.DeviceSerialNumber;
     end
       
        detector_id = info_dicom_blinded.DetectorID;
        ...
        elseif strcmp(room_id,'H1KRHR84e2d052')
            field{6}='41'; %UCSF_ZM5
        elseif (strcmp(detector_id,'MM65475') || strcmp(detector_id,'CM636339'))  %SELENIABC_3
            field{6}='58';  %Moffitt Selenia_1 'MM601171'
        elseif strcmp(detector_id,'MM601171')
            field{6}='59';    
        elseif strcmp(detector_id,'YM632131') %SELENIABC_3
            field{6}='60';                        
        else            
            field{6}='9999';
        end
