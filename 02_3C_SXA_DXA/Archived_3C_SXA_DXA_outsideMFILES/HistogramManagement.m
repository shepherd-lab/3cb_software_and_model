...
switch ActionRequested
    ...
    case 'DrawHistLine'  %draw the Histogram and the 3 bars
        if Analysis.Step>=1;
            ...
            Hist.Handleline2=plot([Hist.xmax Hist.xmax-10],[0 Hist.maxGraphY],'EraseMode','xor','linewidth',3,'color','blue','ButtonDownFcn','HistogramManagement(''BarMoved'',2)');
    ...
    case 'ComputeHistogram'
        ...
        bin3 = [0:0.01:1];
        bin2 =  double(Image.minOriginalImage) + bin3*(double(Image.maxOriginalImage)-double(Image.minOriginalImage));
      %  figure; imagesc(Image.OriginalImage); colormap(gray);
        mmax = max(max(Image.OriginalImage));
        mmin = min(min(Image.OriginalImage));
