...
global data Database Info ctrl figuretodraw ReportText Error Image ROI  PhantomDetectionFailure
global PowerPointReport AutomaticAnalysis Correction Analysis Recognition Phantom h_init h_slope
...
switch RequestedAction
    ...
    case 'ADDREPORTTEXT'
      ...
      case 'ADDSTEPPOFILES'   
        ...
        % set(foe, 'Visible','off');
         plot(Analysis.signal(15:end,1), Analysis.signal(15:end,2)); hold on; %2:5
        % legend('center', 'upper','down','fit');
         ...
         plot(Analysis.Xcoord*Analysis.Filmresolution, Analysis.Lean_ref_profile,'-b');
           grid on;
         hold off; 
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
         ...
         figure(h_init);
          ...
          PowerPoint('copypastefigure','position',[0.5 0.1 0.45 0.4],'LockAspectRatio',false);delete(h_init); 
          figure(h_slope);
          ...
          grid on;
          PowerPoint('copypastefigure','position',[0.1 0.55 0.4 0.4],'LockAspectRatio',false);delete(h_slope); 
         %%%%%%%%%%% add horizontal profile
         sz = size(Analysis.signal_horiz)
         foe=figure; 
         scrsz = get(0,'ScreenSize');
         set(h_init,'Position',[1 scrsz(4)*3/8 scrsz(3)*2/8 scrsz(4)*3/8]);
        % set(foe, 'Visible','off');
         ...
         plot(Analysis.signal_horiz(1:end,1), Analysis.Leanref_profile_horiz(1:end-1),'-m');
         set(gca,'XLim',[Analysis.signal_horiz(1,1) Analysis.signal_horiz(end,1)]);
         grid on;
         hold off; 
         PowerPoint('copypastefigure','position',[0.5 0.5 0.45 0.45],'LockAspectRatio',false);delete(foe);  
    ...
    case 'SAVECLOSE'
            PowerPoint('saveclose','text',['C:\Temp\AutomaticAnalysis',PowerPointReport.PatientIDbeginning,'_',num2str(param),'.ppt']);
end