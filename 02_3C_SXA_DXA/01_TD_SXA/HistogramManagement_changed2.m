...
switch ActionRequested
    ...
    case 'DrawHistLine'  %draw the Histogram and the 3 bars
        if Analysis.Step>=1;
            ...
            ab = Image.maximage
            set(Hist.Handle,'YTick',[],'Xlim',[Image.immin Image.maximage],'Ylim',[0 Hist.maxGraphY]);    
    ...
    case 'ComputeHistogram'
        ...
        Hist.values=histc(reshape(Image.image,1,prod(size(Image.image))),bin);
    ...
    case 'DetectMoveement'
        CurrentPoint=get(Hist.Handle,'CurrentPoint');CurrentX=CurrentPoint(1,1)*100/(Image.maximage - Image.immin);
        if select==0
