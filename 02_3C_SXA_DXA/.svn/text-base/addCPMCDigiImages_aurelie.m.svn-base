...
parentdir = uigetdir('Y:\Breast Studies\Tlsty_P01_invivo\', 'Select the folder you want to add to the Database'); % displays a dialog box enabling to choose the directory to read the filesfrom.
...
% %         field{4}=cell2mat(loadDataSetup(3,1));
        field{4}='';
        field{5}=info_dicom.AcquisitionDate;  %date_acquisition % Al removed StudyDate
        len=length(info_dicom.StationName);
        room_id=info_dicom.StationName(len);
        len1=length(info_dicom.InstitutionName);
        serial_number = info_dicom.DeviceSerialNumber;
        if len1<2
            room_id1='';
        else
        ...
        elseif strcmp(room_id,'6')|| strcmp(room_id1,' 2')
            field{6}='6';
        elseif strcmp(room_id,'7') || strcmp(room_id1,' 6')
            field{6}='7';
        elseif strcmp(serial_number,'H1KRHR8416ac98') % Selenia Research ZM-10 @ WHC
            field{6}='116';
        end
