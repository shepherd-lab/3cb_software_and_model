function  Z4ComputeBreastDensity(Innerline,ROI, Image)
 ...
 %  figure; imagesc(CornerMask); colormap(gray);
  ...
  % figure; imagesc((~CornerMask)&MaskROIproj); colormap(gray);
   ...
   Analysis.SXADensityImageSkinCrop=MaskROIproj.*DensityImageSkin; %
   Analysis.SXADensityImageCrop = DensityImage;
   ...
   if corner_area < breast_area/10
       ...
       Analysis.DensityPercentageSkin=nansum(nansum(DensityImageSkin.*MaskROIprojCorner.*thickness_mapprojCrop))/sum(sum(MaskROIprojCorner.*thickness_mapprojCrop));
       Analysis.DensityPercentageTotal=nansum(nansum(DensityImage.*BreastMask.*thickness_mapproj))/sum(sum(BreastMask.*thickness_mapproj));
       Analysis.DensityPercentage=nansum(nansum(DensityImage.*MaskROIprojCorner.*thickness_mapprojCrop))/sum(sum(MaskROIprojCorner.*thickness_mapprojCrop));
   else
       Analysis.DensityPercentageNoCorr=nansum(nansum(DensityImage.*MaskROIproj.*thickness_mapprojCrop))/sum(sum(MaskROIproj.*thickness_mapprojCrop));
       Analysis.DensityPercentageSkin=nansum(nansum(DensityImageSkin.*MaskROIproj.*thickness_mapprojCrop))/sum(sum(MaskROIproj.*thickness_mapprojCrop));
       ...
       Analysis.DensityPercentage=nansum(nansum(DensityImage.*MaskROIproj.*thickness_mapprojCrop))/sum(sum(MaskROIprojCorner.*thickness_mapprojCrop));
   end
  ...
  % figure; imagesc(im_implant); colormap(gray);
   ...
   Tmask3C(ROI.ymin:ROI.ymin+ROI.rows-1,ROI.xmin:ROI.xmin+ROI.columns-1) = Analysis.SXAThicknessImageTotal;
   Tmaskones(ROI.ymin:ROI.ymin+ROI.rows-1,ROI.xmin:ROI.xmin+ROI.columns-1)= BreastMask;
