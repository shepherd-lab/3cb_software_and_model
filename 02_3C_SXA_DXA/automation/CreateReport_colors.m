...
% % %          clear roicc xy flip_image;
        ...
        plot(xy(:,1)-roi.xmin+roi.columnsk, xy(:,2)-roi.ymin+roi.rowsk, '-g');
        colmaps.watercc_xy = [xy(:,1)-roi.xmin+roi.columnsk, xy(:,2)-roi.ymin+roi.rowsk];
         %PowerPoint('addtext','text',['Lipid ' ],'position',[Position(1)+0.55 Position(2)]);
       ...
       %roicc  = medfilt2(roicc , [5 5]);
        if (strcmp(patient_id(3:7),'02019')  | strcmp(patient_id(3:7),'02012')) %strcmp(patient_id(3:4),'01')
         ...
         plot(xy(:,1)-roi.xmin+roi.columnsk, xy(:,2)-roi.ymin+roi.rowsk, '-g');
         colmaps.lipidcc_xy = [xy(:,1)-roi.xmin+roi.columnsk, xy(:,2)-roi.ymin+roi.rowsk];
        % PowerPoint('addtext','text',['Lipid ' ],'position',[Position(1)+0.55 Position(2)]);
        ...
        plot(xy(:,1)-roi.xmin+roi.columnsk, xy(:,2)-roi.ymin+roi.rowsk, '-g');
         colmaps.protcc_xy = [xy(:,1)-roi.xmin+roi.columnsk, xy(:,2)-roi.ymin+roi.rowsk];
        % PowerPoint('addtext','text',['Lipid ' ],'position',[Position(1)+0.55 Position(2)]);
...
% %          PowerPoint('copypastefigure','position',[0.4 0.162 0.1 0.1],'LockAspectRatio',false);delete(foe);  
       ...
       %roicc  = medfilt2(roicc , [5 5]);
        ...
        plot(xy(:,1)-roi.xmin+roi.columnsk, xy(:,2)-roi.ymin+roi.rowsk, '-g');
        colmaps.waterml_xy = [xy(:,1)-roi.xmin+roi.columnsk, xy(:,2)-roi.ymin+roi.rowsk];
         %PowerPoint('addtext','text',['Lipid ' ],'position',[Position(1)+0.55 Position(2)]);
...
%          
        ...
        plot(xy(:,1)-roi.xmin+roi.columnsk, xy(:,2)-roi.ymin+roi.rowsk, '-g');
         colmaps.lipidml_xy = [xy(:,1)-roi.xmin+roi.columnsk, xy(:,2)-roi.ymin+roi.rowsk];
        % PowerPoint('addtext','text',['Lipid ' ],'position',[Position(1)+0.55 Position(2)]);
...
% %         foe=figure;imagesc(roicc);colormap(gray); h=colorbar;set(h,'fontsize',14); hold on; %,clims_water
        ...
        plot(xy(:,1)-roi.xmin+roi.columnsk, xy(:,2)-roi.ymin+roi.rowsk, '-g');
         colmaps.protml_xy = [xy(:,1)-roi.xmin+roi.columnsk, xy(:,2)-roi.ymin+roi.rowsk];
         %PowerPoint('addtext','text',['Lipid ' ],'position',[Position(1)+0.55 Position(2)]);
         PowerPoint('copypastefigure','position',[0.85 0.162 0.1 0.1],'LockAspectRatio',false);delete(foe);  
         clear roicc;
         fname_colors = ['\\researchstg\aaStudies\Breast Studies\3CB R01\Presentations\RSNA\',patient_id,'.mat'];
         save(fname_colors,'colmaps');
         a = 1;
...
% %           PowerPoint('addtext','text',[''],'position',[0.02 Position1(2)+0.04],'carriage',0.5);  
       ...
       % sz = size(results_3CB);
        if ~isempty(maps_CC.results) %& sz(1)>= 1
            PowerPoint('addtext','text',['Lesion ROI water/lipid/protein, cm:'],'position',[Position(1) Position1(2)]); PowerPoint('addtext','text',[ sprintf('%.2f',cc.ROIW),'/', sprintf('%.2f',cc.ROIL),'/', sprintf('%.2f',cc.ROIP)],'position',[Position(1)+0.4 Position1(2)]); % /total  ,'/',num2str(results_3CB(1,4))
            PowerPoint('addtext','text',[ sprintf('%.2f',ml.ROIW),'/', sprintf('%.2f',ml.ROIL),'/', sprintf('%.2f',ml.ROIP)],'position',[Position(1)+0.6 Position1(2)],'carriage',1.0); %,'/',num2str(results_3CB(1,4))
            Position1=PowerPoint('gettextposition');
        else
         ...
         if ~isempty(maps_CC.results) %& sz(1)>= 1
            PowerPoint('addtext','text',['Periphery water/lipid/protein, cm:'],'position',[Position(1) Position1(2)]); PowerPoint('addtext','text',[sprintf('%.2f',cc.bkgrW1),'/',sprintf('%.2f',cc.bkgrL1),'/',sprintf('%.2f',cc.bkgrP1)],'position',[Position(1)+0.4 Position1(2)]); % /total  ,'/',num2str(results_3CB(1,4))
            PowerPoint('addtext','text',[sprintf('%.2f',ml.bkgrW1),'/',sprintf('%.2f',ml.bkgrL1),'/',sprintf('%.2f',ml.bkgrP1)],'position',[Position(1)+0.6 Position1(2)],'carriage',1.0); %,'/',num2str(results_3CB(1,4))
            Position1=PowerPoint('gettextposition');
        else
            PowerPoint('addtext','text',['Breast water/lipid/protein, cm:'],'position',[Position(1) Position1(2)]); PowerPoint('addtext','text',' -/-/-/-','position',[Position(1)+0.4 Position1(2)],'carriage',1.0); %/total
            Position1=PowerPoint('gettextposition');
        ...
        if ~isempty(maps_CC.results) & strcmp(patient_id(3:4),'01')
            PowerPoint('addtext','text',['Breast water/lipid/protein, cm:'],'position',[Position(1) Position1(2)]); PowerPoint('addtext','text',[sprintf('%.2f',cc.breastW),'/',sprintf('%.2f',cc.breastL),'/',sprintf('%.2f',cc.breastP)],'position',[Position(1)+0.4 Position1(2)]); % /total  ,'/',num2str(results_3CB(1,4))
            PowerPoint('addtext','text',[sprintf('%.2f',ml.breastW),'/',sprintf('%.2f',ml.breastL),'/',sprintf('%.2f',ml.breastP)],'position',[Position(1)+0.6 Position1(2)],'carriage',1.0); %,'/',num2str(results_3CB(1,4))
            Position1=PowerPoint('gettextposition');
        else
            PowerPoint('addtext','text',['Breast water/lipid/protein, cm:'],'position',[Position(1) Position1(2)]); PowerPoint('addtext','text',' -/-/-/-','position',[Position(1)+0.4 Position1(2)],'carriage',1.0); %/total
            Position1=PowerPoint('gettextposition');
