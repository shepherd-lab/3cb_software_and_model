function CalibrationDXA_3C_fat_water_cubic_26pointsAUTO(type)
global X patient_ID file Image Info Database Analysis3C
...
%CalibrationPoints=load('\\ming\aaDATA\Breast Studies\3C_data\20100311\20100311_25_CalibrationPoints.txt');
 %SQLstatement=['select Calibration_filename from acquisition where patient_id like ''',patient_ident(1:7),''' and date_acquisition like ''',date_acq(1:8),''' and SUBSTRING(fn,1,4) like ''',fn_he,''''];
% % % %  Info.AcquisitionKey = 1881;
% % % %  Database.Name = 'mammo_DXA';
 ...
 calib_fname  = char(mxDatabase(Database.Name,SQLstatement));
 chstr = strtrim(calib_fname )
 calib_fname = [chstr,'.txt'];
 [CalibrationPoints, Info.calib_comments] = read_file(calib_fname)
...
% % % % % dev_tprotein = (sum((Data(:,6)-Result(:,3)).^2)./size(Data,1)).^0.5
 ...
 Analysis3C.water_error=dev_twater/max(Data(:,1))*100
 Analysis3C.fat_error=dev_tfat/max(Data(:,2))*100
 Analysis3C.protein_error=dev_tprotein/max(Data(:,6))*100
