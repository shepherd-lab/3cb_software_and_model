...
%------------- BEGIN CODE --------------
function update_3CResults_auto_v1()
global ROI Image patient_ID flip_info FreeForm file MaskROI_breast maps flag Info Threshold Analysis
...
list_calc_file = 'Z:\Breast Studies\3C_data\RO1_3Cimages\UCSF\Results\Auto_3C_analysis_runs\pat_list_microcalc.txt';
% list_calc_file = 'Z:\Breast Studies\3C_data\RO1_3Cimages\UCSF\Results\Auto_3C_analysis_runs\pat_list_calc.txt'

% \\researchstg\aaData
% Z:\Breast Studies\3C_data\RO1_3Cimages\UCSF\Results\Auto_3C_analysis_runs

%file_read = dir(input_dir);
pat_list = textread(list_calc_file,'%s');
view_id  = ['CC'; 'ML'];

output = [];

run_number = 2;
for ii = 1:length(pat_list)
 for ik = 1:2    
     try
 patient_ID = char(pat_list(ii));
 if ~isempty(strfind(patient_ID,'3C01')) |  ~isempty(strfind(patient_ID,'3CB'))
     input_dir = '\\researchstg\aaData\Breast Studies\3C_data\RO1_3Cimages\UCSF\Results\Auto_3C_analysis_runs\Run2';
     center = 'ucsf';
 elseif ~isempty(strfind(patient_ID,'3C02'))
     input_dir = '\\researchstg\aaData\Breast Studies\3C_data\RO1_3Cimages\MOffitt\Results\Auto_3C_analysis_runs\Run2';
     center = 'moff';
 else
     input_dir = [];
 end    
...
%  maps.LEPres= Image.LEPresFlipped(ROI.ymin:ROI.ymin+ROI.rows-1,ROI.xmin:ROI.xmin+ROI.columns-1);  
 fname = ['\3CBResults_',patient_ID,'_',view_id(ik,:),'_SM_run',num2str(run_number),'.mat'];
 fname_results = [input_dir,fname];
 load(fname_results);
 MaskROI_breast = maps.breast_mask;
 MaskROI_breast(isnan(MaskROI_breast)) = 0;
 
lesion_mask = zeros(size(MaskROI_breast));
 num_lesions = length( maps.lesion(1,:))
 lesion_ROI2 = [];
for i = 1:num_lesions %.xy(:,1))
 lesion_ROI2(:,:,i) = double(roipoly(maps.LEPres,maps.lesion(1,i).xy(:,1),maps.lesion(1,i).xy(:,2))).*MaskROI_breast; 
 lesion_mask = lesion_mask | lesion_ROI2(:,:,i);
end

distbw =  bwdist(lesion_mask);
distbw = distbw*0.014;
distbw_thresh3 = distbw<0.6;
s = regionprops(distbw_thresh3,'BoundingBox');

roi.ymin = round(s.BoundingBox(1));
roi.ymax = round(s.BoundingBox(1))+s.BoundingBox(3);
roi.xmin = round(s.BoundingBox(2));
roi.xmax = round(s.BoundingBox(2))+s.BoundingBox(4);

% roi.ymin = min(maps.lesion(1,i).xy(:,1));
% roi.ymax = max(maps.lesion(1,i).xy(:,1));
% roi.xmin = min(maps.lesion(1,i).xy(:,2));
% roi.xmax = max(maps.lesion(1,i).xy(:,2));
Analysis.filename = fname;
Analysis.Surface = [];
Image.image = maps.LEPres.*MaskROI_breast;
se = strel('disk',20);
temproi = imtophat(Image.image,se);
temproi = temproi.*distbw_thresh3;
Image.image=temproi(roi.xmin:roi.xmax,roi.ymin:roi.ymax);
im_sz = size(Image.image);
% Image.image = imresize(Image.image,2);
Image.maximage2 = max(max(Image.image));
Image.immax = max(max(Image.image));
Image.immin = min(min(Image.image));

Analysis.Step = 1;
% Image.image = maps.LEPres;

ReinitImage(Image.image); %,'OPTIMIZEHIST'
HistogramManagement('ComputeHistogram',3);
HistogramManagement('ThresholdManagement',3);

%  maps.mask_calc = calc_segment(maps.LEPres, maps.LEatten, lesion_mask);
maps.mask_calc = ones(size(MaskROI_breast));
maps.mask_calc(roi.xmin:roi.xmax,roi.ymin:roi.ymax) = double(maps.mask_calc2);
maps.mask_calc2 = [];
maps.calc_threshold =Threshold.value;

% MaskROI_breast = MaskROI_breast.*maps.mask_calc;
 
 for i=1:num_lesions   %length(FreeForm.FreeFormCluster(1,:))
lesion_ROI = lesion_ROI2(:,:,i).*maps.mask_calc;
...
% [lesion_mask, outer_1_mask, outer_2_mask, outer_3_mask] = get_lesion_masks(xp, yp, xRes, yRes, m_ROI, n_ROI)
 ...
 distbw =  bwdist(lesion_ROI2(:,:,i));
 distbw = distbw*0.014;
 distbw_thresh1 = double(distbw<0.2);
 lesion_periph1 = double(~(~lesion_ROI - distbw_thresh1)).*MaskROI_breast.*maps.mask_calc;
  distbw_thresh2 = double(distbw<0.4);
 lesion_periph2 = double(~(~(lesion_ROI + lesion_periph1) - distbw_thresh2)).*MaskROI_breast.*maps.mask_calc;
  distbw_thresh3 = double(distbw<0.6);
 lesion_periph3 = double(~(~(lesion_ROI + lesion_periph1 + lesion_periph2) - distbw_thresh3)).*MaskROI_breast.*maps.mask_calc;
...
%  maps.results.negROIW = length(zindex)/length(totind)*100;
 ...
 s = regionprops(lesion_ROI2(:,:,i),'Area');
...
%  elseif ~isempty(findstr(center,'ucsf_pilot'))
...
results3C{1,	189	} =	maps.calc_threshold;
%%
 save(fname_results,'maps');
%  maps.results
 update_3CtoExcel(results3C,center);
%  copyfile(fname_results,results_dir);
%  create_report(); % for calcification segmentation visualization
 end
     catch
         output = [output;patient_ID]
     end
 end
end
a = 1;
