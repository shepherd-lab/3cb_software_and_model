...
global X_angle ROI MachineParams Analysis Outline thickness_ROI Info thickness_mapproj  thickness_mapreal breast_Maskcorr PreciseOutline Error Image
global flag BreastMask inner_mask I0mask

% Developed by APM 09092015
% Copy right BBDG 2105
...
% % %      thickness_ROI = thickness_ROIcreation(X_angle,Y_angle); 
     thickness_ROI = thickness_ROIcreation;
   ...
   % [outline_xcorr,outline_ycorr] = SkinDetection_corrected(Outline.x',Outline.y' ,midpoint,thickness_ROI);
    ...
    % [outline_xcorr,outline_ycorr] = SkinDetection_corrected(xt, yt,midpoint,thickness_ROI)
     breast_Maskcorr = (1 - roipoly(ROI.image ,outline_xcorr,outline_ycorr));% ROI.image
...
% % %          BreastMask=BreastMask(ROI.row_start:ROI.row_end, ROI.column_start:ROI.column_end);

%          BreastMask=BreastMask(ROI.ymin:ROI.ymin+ROI.rows-1,ROI.xmin:ROI.xmin+ROI.columns-1); % correct the size
%          

     
% % %       if flag.Pectoral_MLOView    
% % %      
% % % %      Image.Mask_MLO=Image.Mask_MLO(ROI.ymin:ROI.ymin+ROI.rows-1,ROI.xmin:ROI.xmin+ROI.columns-1); 
% % %      
...
% % %           any_zeros = any(I0mask==1, 1); % checks every column to see if any elements in column are non-zero
% % %          column_start = find(any_zeros==1, 1); % first column that contains 1
% % %          column_end = find(any_zeros==1, 1, 'last'); % last column that contains 1
% % %          
% % %          any_zeros = any(I0mask==1, 2); % checks every row to see if any elements in row are non-zero
% % %          row_start = find(any_zeros==1, 1); % first row that contains 1
% % %          row_end = find(any_zeros==1, 1, 'last'); % last row that contains 1
% % %          
% % %          % final image has excess "white" on edges eliminated
% % %          BreastMask = I0mask(row_start:row_end, column_start:column_end);
% % %          
% % %          
% % %          any_zeros = any(inner_mask==1, 1); % checks every column to see if any elements in column are non-zero
% % %          column_start = find(any_zeros==1, 1); % first column that contains 1
% % %          column_end = find(any_zeros==1, 1, 'last'); % last column that contains 1
% % %          
% % %          any_zeros = any(inner_mask==1, 2); % checks every row to see if any elements in row are non-zero
% % %          row_start = find(any_zeros==1, 1); % first row that contains 1
% % %          row_end = find(any_zeros==1, 1, 'last'); % last row that contains 1
% % %          
% % %          % final image has excess "white" on edges eliminated
% % %          inner_mask = inner_mask(row_start:row_end, column_start:column_end);
     
      
     szmpr = size(BreastMask);
     szbm = size(inner_mask);
     ...
     ybrmax = min([szmpr(1) szbm(1)]);
     
     BreastMask = BreastMask(1:ybrmax,:);
     
     end
     

% % %      if flag.Pectoral_MLOView  |szmpr(2)<400  % sometimes breast is too small and we can't use inner_mask
% % % 
% % %          inner_mask=BreastMask;
% % %          
% % %      else
% % %          if nnz(inner_mask)<50  % if there is no inner mask use the whole breast 
% % %              
% % %              inner_mask=BreastMask;
% % %             
% % %          else
% % % 
% % %               inner_mask=inner_mask.*BreastMask;
% % %              
% % %          end
% % %          
% % %      end
...
% % %          
% % %          % final image has excess "white" on edges eliminated
% % %          inner_mask_temp = inner_mask(row_start:row_end, column_start:column_end);
% % %          ymin = min(find(inner_mask_temp(:,1)==1));
% % %          ymax = max(find(inner_mask_temp(:,1)==1));
% % %      end
     
     
     %%%% This error happening during MLO view Analysis
     try
         % % % %      if(~isempty(ymin)) & (~isempty(ymax ))
         
         boundary = bwtraceboundary(inner_mask_temp,[ymin,1],'E');
         
         % % % %      else
         
         
         
         % % %                  figure;imagesc(inner_mask_temp);colormap(gray)
         % % % %                  s=size(inner_mask_temp);
         % % % %                  for row = 2:55:s(1)
         % % % %                      for col=1:s(2)
         % % % %                          if inner_mask_temp(row,col),
         % % % %                              break;
         % % % %                          end
         % % % %                      end
         
         % % % %          boundary = bwtraceboundary(inner_mask_temp, [row, col], 'E');
         % % %                      if(~isempty(boundary))
         ...
         ymin = min(find(BreastMask(:,1)==1));
         ymax = max(find(BreastMask(:,1)==1));
         boundary = bwtraceboundary(BreastMask,[ymin,1],'E');
%          inner_mask=BreastMask;
         % % %           boundary=Analysis.Boundary_MLO;
     end
...
% % % %      end
     ...
     % % % (isempty(ymin)) & (isempty(ymax ))          % sometimes in MLO view the inner mask is very small and we should start looking for white pixel in 300 or obove
     % % %              ymin = min(find(inner_mask(:,300)==1));
     % % %              ymax = max(find(inner_mask(:,300)==1));
...
% % %      thickness_ROI = thickness_ROI(row_start:row_end, column_start:column_end);
% % %      thickness_mapreal = thickness_mapreal(row_start:row_end, column_start:column_end);
...
thickness_mapproj = thickness_mapproj.*BreastMask;
