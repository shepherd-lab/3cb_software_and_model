...
global Threshold visu ROI  Image Error Outline Analysis ChestWallData
global BreastMask
%thcnt = 1
%tic
%try
...
%    if isfield(ChestWallData,'Curve')
%       ChestWallData.Curve  = [ChestWallData.Curve(:,1)-ROI.ymin, ChestWallData.Curve(:,2)-ROI.xmin];
%    end
...
%    figure;imagesc(chestwall_Mask); colormap(gray);
   %
   min_temproi = min(min(temproi));
   max_temproi = max(max(temproi));
   BW=((temproi/(max(max(Image.maximage)))*Image.colornumber+0)>Threshold.value*Image.colornumber+0).* BreastMask;
   %BW=((temproi/(max(max(temproi))-min(min(temproi)))*Image.colornumber)>Threshold.value*Image.colornumber).* BreastMask;
    clear temproi;
...
% %      figure;imagesc(BW); colormap(gray);
   ...
   % toc
    ...
    %Threshold.boundary = [Threshold.boundary(:,1)-ROI.ymin, Threshold.boundary(:,2)-ROI.xmin];
