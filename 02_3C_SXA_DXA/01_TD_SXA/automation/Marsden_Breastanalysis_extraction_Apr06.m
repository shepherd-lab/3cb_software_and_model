...
% % %     for index=1
      ...
      % SQLStatement ='SELECT dbo.acquisition.*, dbo.QCWAXAnalysis.*, dbo.QCwaxSXAPhantomInfo.* FROM dbo.QCwaxSXAPhantomInfo INNER JOIN dbo.QCWAXAnalysis ON dbo.QCwaxSXAPhantomInfo.QCWAXAnalysis_id = dbo.QCWAXAnalysis.QCWAXAnalysis_id INNER JOIN dbo.commonanalysis ON dbo.QCWAXAnalysis.commonanalysis_id = dbo.commonanalysis.commonanalysis_id INNER JOIN dbo.acquisition ON dbo.commonanalysis.acquisition_id = dbo.acquisition.acquisition_id WHERE (dbo.QCWAXAnalysis.QC_analysis_date LIKE ''%-Dec-2011%'') AND (dbo.acquisition.machine_id = 39)';
     % SQLStatement =['SELECT dbo.acquisition.*, dbo.SXAStepAnalysis.*, dbo.OtherSXAStepInfo.* FROM dbo.OtherSXAStepInfo INNER JOIN dbo.SXAStepAnalysis ON dbo.OtherSXAStepInfo.SXAStepAnalysis_id = dbo.SXAStepAnalysis.SXAStepAnalysis_id INNER JOIN dbo.commonanalysis ON dbo.SXAStepAnalysis.commonanalysis_id = dbo.commonanalysis.commonanalysis_id INNER JOIN dbo.acquisition ON dbo.commonanalysis.acquisition_id = dbo.acquisition.acquisition_id WHERE (dbo.SXAStepAnalysis.SXA_analysis_date LIKE ''%May-2012%'') AND dbo.acquisition.acquisition_id =',num2str(ACQIDList(index)),'order by dbo.SXAStepAnalysis.sxa_analysis_date'];
...
% %       
      SQLStatement =['SELECT dbo.acquisition.*, dbo.sxastepanalysis.*, dbo.OtherSXAStepInfo.*',... 
                    ...
                    ' INNER JOIN dbo.acquisition ON dbo.commonanalysis.acquisition_id = dbo.acquisition.acquisition_id',...
                      ' WHERE (dbo.sxastepanalysis.sxa_analysis_date LIKE ''%Dec-2012%'') and (dbo.acquisition.film_identifier like ''%Marsden_11-07-2012%'') and dbo.sxastepanalysis.run_number = 11 AND dbo.acquisition.acquisition_id =' ,num2str(1000876701),...
                    '  ORDER BY dbo.sxastepanalysis.sxa_analysis_date']
                    
                   % ' WHERE ((dbo.sxastepanalysis.sxa_analysis_date LIKE ''%Dec-2012%'') AND (dbo.acquisition.film_identifier LIKE ''%Marsden_11-07-2012%'') AND Run_number = 11) AND dbo.acquisition.acquisition_id =' ,num2str(1000876701),...
                   % '  ORDER BY dbo.sxastepanalysis.sxa_analysis_date'];
...
% % %                     ' INNER JOIN dbo.commonanalysis ON dbo.sxastepanalysis.commonanalysis_id = dbo.commonanalysis.commonanalysis_id',...
% % %                     ' INNER JOIN dbo.acquisition ON dbo.commonanalysis.acquisition_id = dbo.acquisition.acquisition_id',...
% % %                     ' WHERE ((dbo.sxastepanalysis.sxa_analysis_date LIKE ''%23-May-2012%'') or (dbo.sxastepanalysis.sxa_analysis_date LIKE ''%24-May-2012%'')) AND dbo.acquisition.acquisition_id =' ,num2str(ACQIDList(index)),...
% % %                     '  ORDER BY dbo.sxastepanalysis.sxa_analysis_date'];
                 ...
                 SQLStatement =['SELECT dbo.acquisition.*, dbo.sxastepanalysis.*, dbo.OtherSXAStepInfo.*',... 
                    ...
                    ' INNER JOIN dbo.commonanalysis ON dbo.sxastepanalysis.commonanalysis_id = dbo.commonanalysis.commonanalysis_id',...
                    ' INNER JOIN dbo.acquisition ON dbo.commonanalysis.acquisition_id = dbo.acquisition.acquisition_id',...
                     ' WHERE (dbo.sxastepanalysis.sxa_analysis_date LIKE ''%Dec-2012%'') and (dbo.acquisition.film_identifier like ''%Marsden_11-07-2012%'') and dbo.sxastepanalysis.run_number = 11 ',...
                    '  ORDER BY dbo.sxastepanalysis.sxa_analysis_date']
                    
                    %' WHERE ((dbo.sxastepanalysis.sxa_analysis_date LIKE ''%Dec-2012%'') AND (dbo.acquisition.film_identifier LIKE ''%Marsden_11-07-2012%'') AND Run_number = 11) ',...
                    %'  ORDER BY dbo.sxastepanalysis.sxa_analysis_date'];
