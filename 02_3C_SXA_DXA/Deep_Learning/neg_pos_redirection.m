function pilot_conversion()
 ...
 count = 0;
  ...
  output_pos_dir = 'F:\Dream_contest2\Training_Nikulin\DDSM_1\src\DICOMS_3CBpres_noSXA\pos';
  output_neg_dir = 'F:\Dream_contest2\Training_Nikulin\DDSM_1\src\DICOMS_3CBpres_noSXA\neg';
...
%                ind = cell2mat(strfind(txt,file_names(i).name(1:end-4)));
              ...
              if ~isempty(ind)    
                  if num(ind) == 1
                    copyfile(full_filename,output_pos_dir);  
                    count = count + 1
                  else
                   copyfile(full_filename,output_neg_dir); 
                    count = count + 1  
                  end
              else
