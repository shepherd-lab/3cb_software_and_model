function [count, len] = addqacodes_inDatabase2
%global Database
...
for i = 1:len
  ...
  qacodes = mxDatabase(database_name, ['SELECT ALL qa_code_results.QA_Code FROM acquisition,qa_code_results WHERE acquisition.acquisition_id = qa_code_results.acquisition_id  AND acquisition.acquisition_id =',  str_list]);  
  qacodes_array = cell2mat(qacodes);
  index = find(qacodes_array==QAcodeNumber);
    if isempty(index)
      add_recordIN(database_name,'QA_code_results',[{str_list},{num2str(QAcodeNumber)}]);
      count = count + 1;
    end
end
num = count
