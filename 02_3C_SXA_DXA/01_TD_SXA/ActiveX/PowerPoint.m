...
global PowerPointData  figuretodraw axestodraw copyfigure_handle PathName FileName_list image_filename
try 
...
switch RequestedAction
    ...
    case 'copypastefigure'
        ...
        %figure(current_fighandle);
        
        %print  opt -r100 -dmeta
        %print  -f4 -r100 -dmeta
       % h = gcf;
        %h1 = figuretodraw_separated;
       % h2 = axestodraw_separated;
        %figure(h1);
       %axes(axestodraw_separated);
        %opt = ['-f',num2str(h)];
        %h2 = findobj('Tag','BreastImage');
       % figure(copyfigure_handle);
       % print(copyfigure_handle,  '-r100', '-dbitmap');%'-dmeta');dbitmap axestodraw
       %image_filename = [acqs_filename(1:end-4),'image.emf']; 
% % %        image_filename =
% [PathName,FileName_list(1:end-4),'image.emf']; 
       image_filename = [PathName,FileName_list(1:end-4),'image.emf']; 
       print(copyfigure_handle, '-dmeta',image_filename);%'-r100',
        
        %figure(gcf);
        % print(h,  '-r100', '-dmeta');
        %h = gcf;
        %Shapes = PowerPointData.new_slide.Shapes;
        %Shapes.AddPicture('D:\users\smalkov\temp_image.bmp',01,,150,18,300,235);
        
        %PowerPointData.pic1 = invoke(PowerPointData.new_slide.Shapes,'Paste');
       % PowerPointData.pic1 = invoke(PowerPointData.new_slide.Shapes,'AddPicture','D:\users\smalkov\temp_image.bmp');
        pic2 = invoke(PowerPointData.new_slide.Shapes,'AddPicture',image_filename,0,1,LEFT,TOP,WIDTH,HEIGHT);
        %figure; imagesc(PowerPointData.pic1); colormap(gray);
