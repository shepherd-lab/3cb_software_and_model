...
if machine ~=8 %(machine == 4 | machine == 9 |machine == 13 | machine == 1 |len == 9)
    if Analysis.PhantomID == 9 & (len == 8 )
        ...
        %group1 = temp_gr1(1:3,:);
        %gr2 - bottom
        mm1 = (middle+70*crop_coef);
        mm2 = (max(b(:,1))-80*crop_coef);
        gr2 = b(b(:,2)>mm1 & b(:,1)<mm2,:);
        ...
        elseif sz2(2) == 3 %upper 312
            ...
            gr3 = b(b(:,1)>group1(3,1)+20*crop_coef,:);
            gr3 = b(b(:,1)>max([group1(3,1) group2(3,1)]) + 20*crop_coef,:);
            temp_gr3 = sortrows(gr3,2);
            group3 = temp_gr3(1:2,:);
% %             k = abs( (group1(2,1) - group2(2,1))/(group3(2,1) - group3(1,1)));
% %             if  k < 3
% %                  index3 = [8;9];
% %             elseif k > 3 & min([group3(2,1) - group3(1,1)]) < (group1(2,1) - group2(2,1))/2
% %                  index3 = [7;9];
% %             else
% %                 index3 = [9;7];
% %             end
            meany_gr2 = mean(temp_gr2(:,2)); %bottom
            meany_gr1 = mean(temp_gr1(:,2));  %upper
            diff3 = (abs(max(temp_gr3(:,2)) - min(temp_gr3(:,2))));
            mean3 = mean(temp_gr3(:,2));
            diff21 = (meany_gr2-meany_gr1);
            middle = mean([meany_gr2,meany_gr1]);
            if diff21 < 3*diff3 
                index3 = [9;8];
            elseif mean3 >middle
                index3 = [7;8];
...
% %             diff1 = abs(min(temp_gr3(:,2)));
% %             diff2 = abs(max(temp_gr3(:,2)));            
% %             if  max([diff1,diff2]) < diff3
% %                  index3 = [8;9];
% %             elseif diff2 > diff1
% %                  index3 = [7;9];
% %             else
...
% %           end
    ...
    elseif Analysis.PhantomID == 9 & (len == 7 )
        middle = (max(b(:,2))+ min(b(:,2)))/2;
        % gr1 = b(b(:,2)<middle+70*crop_coef,:);  gr1 - upper
        gr1 = b(b(:,2)<middle+70*crop_coef & b(:,1)<(max(b(:,1))-(max(b(:,1))-min(b(:,1)))*0.21),:);   %   -80*crop_coef),:); %change 100 for 80 for GE machine
        temp_gr1 = sortrows(gr1,1);
        ...
        elseif sz2(1) == 2
            ...
            gr3 = b(b(:,1)>group1(3,1)+28*crop_coef,:); %20
            temp_gr3 = sortrows(gr3,2);
             group3 = temp_gr3(1:2,:);
...
%             
            ...
            diff1_12d2 =  (group1(1,1) +  group1(2,1))/2;
            diff1_23d2 =  (group1(2,1) +  group1(3,1))/2;
                d12 = group1(2,1) -  group1(1,1);
            d23 = group3(1,1) - group1(2,1);
            ratio23 = d23/d12;
            % %                 diff2_12 =  group2(1,1) -  group2(2,1);
            % %                 diff1_12 =  group1(1,1) -  group2(2,1);
            group2 = temp_gr2(1:2,:);
            index2 = zeros(2,1);
            if  group2(1,1) <  diff1_12d2 && ratio23 < 1.5
                index2(1) = 3; %6;
                if group2(2,1) > diff1_23d2
                    index2(2) = 2; %5;
                else
...
% %           end
    ...
    elseif Analysis.PhantomID == 9 & len == 6
        middle = (max(b(:,2))+ min(b(:,2)))/2;
        % gr1 = b(b(:,2)<middle+70*crop_coef,:);  gr1 - upper
        gr1 = b(b(:,2)<middle+70*crop_coef & b(:,1)<(max(b(:,1))-(max(b(:,1))-min(b(:,1)))*0.21),:);   %   -80*crop_coef),:); %change 100 for 80 for GE machine
        temp_gr1 = sortrows(gr1,1);
