function  UKComputeBreastDensity(Innerline,ROI, Image)
    global Info bb Analysis params roi_values Error h_slope Z4coef_table ROI MachineParams UKcoef_table
    Error.DENSITY=false;
    ...
    %params(4) - Info.bucky_distance;%;1.7; %1.38-0.05
     ...
     h_corrvect = UKcoef_table(:,1);
     klean_vect = UKcoef_table(:,2);
     km_vect = UKcoef_table(:,3);
    ...
    Y_angle = Analysis.ry * 0.9 ;
    X_angle = -Analysis.rx * 0.9; % ??? for sign of angle
   ...
   % roidata_corr = roi_data;
    ...
    len_corr = length(hleanref_vect)
    xdata = hleanref_vect(1:len_corr); %length_80ref
    ydata_ph80 = phleanref_vect(1:len_corr);   %length_80ref
    fresult = fit(xdata,ydata_ph80,'poly2')
    ph80_a2 = fresult.p1;
    ph80_a1 = fresult.p2;
    ph_offset = fresult.p3;
    
    fresult = fit(xdata,ydata_ph80,'poly1')
    ph80_a = fresult.p1;
    ph_b = fresult.p2;
    ...
    for i = 1:len_corr%index%i = 1: length_80ref
       ...
       fatref_vect(i) = klean*(Vph80_fitvect(i) + (1/km-1)*(ph80_a * xdata(i))); % quadratic
       
    end
    Vph80_fitvect  =  Vph80_fitvect';
    %xdata_index = xdata(index);
    ...
    xdata = hleanref_vect(1:len_corr);
    %xdata = xdata(index);
    ...
    Analysis.Fat_ref = klean_thick*(Vph80_thick + (1/km-1)*(ph80_a * thickness ));
   % fatref_vect(i) = klean*(Vph80_fitvect(i) + (1/km-1)*(ph80_a * xdata(i)));
   ...
   %Fat_ref_ROI = klean_ROI*(Vph80_ROI + (1./km_ROI-1)* (Vph80_ROI - ph_offset));    %(ph80_a2 * thickness.^2 + ph80_a1 * thickness));
    ...
    %figure;
    %plot(thickness_ROI,Fat_ref_ROI); 
