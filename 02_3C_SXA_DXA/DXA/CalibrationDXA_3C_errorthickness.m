...
%CalibrationPoints=load('\\ming\aaDATA\Breast Studies\3C_data\20100311\20100311_25_CalibrationPoints.txt');
%%%%%thickness in cm %%%%%%%%%%%%%%%%
Data=[CalibrationPoints(:,1)*0.1 CalibrationPoints(:,2)*0.1  CalibrationPoints(:,3) CalibrationPoints(:,4) CalibrationPoints(:,5)*0.1 ];
...
% % % coef=X(:,1); 
% % % 
% % % water =  coef(1) + coef(2)*(Image.CHE/1000) + coef(3)*Image.CRST + Tmask3C*coef(4) + coef(5)*(Image.CHE/1000).^2 + coef(6)*(Image.CRST).^2 + Tmask3C.^2*coef(7) + coef(8)*((Image.CHE/1000).*Image.CRST) + coef(9)*Tmask3C.*(Image.CHE/1000) + coef(10)*Tmask3C.*Image.CRST + coef(11)*(Image.CHE/1000).^3 + coef(12)*(Image.CRST).^3 + coef(13)*Tmask3C.^3;  
% % % lipid = coef2(1) + coef2(2)*(Image.CHE/1000) + coef2(3)*Image.CRST + Tmask3C*coef2(4) + coef2(5)*(Image.CHE/1000).^2 + coef2(6)*(Image.CRST).^2 + Tmask3C.^2*coef2(7) + coef2(8)*((Image.CHE/1000).*Image.CRST) + coef2(9)*Tmask3C.*(Image.CHE/1000) + coef2(10)*Tmask3C.*Image.CRST + coef2(11)*(Image.CHE/1000).^3 + coef2(12)*(Image.CRST).^3 + coef2(13)*Tmask3C.^3;  

figure; plot(R,HE,'ob');
T1 = T; %-0.1
%A1=[ones(size(Data,1),1) HE R T1 HE.^2 R.^2 T1.^2 HE.*R HE.*T1 T1.*R HE.^3 R.^3 T1.^3];
A1=[ones(size(Data,1),1) HE R T1 HE.^2 R.^2 T1.^2 HE.*R HE.*T1 T1.*R] ; %HE.^3 R.^3 T1.^3]
...
format long g;
%%% for temporary
save([pathName,'\CalibrationCoeffCC_3C01008waxcorrected.mat'],'X'); %_No6cm100
%%%
xc = X
