...
%use RST,HE for the calibration
...
CalibrationData=DrawGrid(ImRST,ImHE);
...
for i1=0:DegreCalibration
    for i2=0:DegreCalibration-i1
        indextot=indextot+1;
        FinalImage=FinalImage+Coef(indextot)*(ImRST.^i1.*ImHE.^i2);
        ThicknessImage=ThicknessImage+Coef2(indextot)*(ImRST.^i1.*ImHE.^i2);
    end
end
