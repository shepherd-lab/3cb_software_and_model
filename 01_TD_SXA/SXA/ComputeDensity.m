...
%end
    ...
    if get(ctrl.CheckBreast,'Value')
        ...
        elseif uint8(Analysis.PhantomID) == 9
            Z4ComputeBreastDensityNew(Outline,ROI, Image);    
        else
            Analysis=funcComputeBreastDensity(ROI,Image,Analysis,Outline,Info);
        end
        set(ctrl.text_zone,'String',strcat('Area surface:',num2str(Analysis.Surface),'pixels - Image Density :',num2str(Analysis.DensityPercentage),'%'));
    else
